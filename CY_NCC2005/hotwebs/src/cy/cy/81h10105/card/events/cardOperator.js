...
// 生成学生上课情况
export function studentBeginSave(props) {
    return new Promise((resolve, rejected) => {
        let flag = props.form.isCheckNow(CARD.form_id) && props.cardTable.checkTableRequired(CARD.table_code);
        if (flag) {
            let data = props.createExtCardData(CARD.page_code, CARD.form_id, [CARD.table_code, CARD.table_code_change, CARD.table_code_parent]);
            props.validateToSave(data, () => {
                ...
                ajax({
                    ...
                    success: (res) => {
                        ...
                        if (success) {
                            toast({ color: STATUS.success, content: props.json['81H10105-000004'] });/* 国际化处理： 保存成功*/
...
// 选中学校后带出额外信息赋值表头
export function getSchooleExtraData(props, data) {
    if (data) {
        ajax({
            url: REQUEST_URL.querySchooleExtraData,
            data: data,
            success: (res) => {
                let { success, data } = res;
                ...
                if (success) {
                    if (res.data && res.data[0] && res.data[0].values) {
                        console.log("res.data[0].values.def10-", res.data[0].values.def10);
                        console.log("res.data-", res.data);
                        if (res.data[0].values.def10) {
                            props.form.setFormItemsValue(CARD.form_id, {
                                ["def10"]: res.data[0].values.def10
                            });
                        }
                        console.log("res.data[0].values.def11-", res.data[0].values.def11);
                        if (res.data[0].values.def11) {
                            console.log("res.data[0].values.def112222-", res.data[0].values.def11);
                            props.form.setFormItemsValue(CARD.form_id, {
                                ["def11"]: res.data[0].values.def11
                            });
                        }
                    }
                }
                    ...
                    success: (res) => {
                        ...
                        if (success) {
                            ...
                            if (res.data) {
                                ...
                                if (res.data.bodys && res.data.bodys[CARD.table_code_change]) {
                                    props.cardTable.setTableData(CARD.table_code, res.data.bodys[CARD.table_code_change]);
                                }
                                if (res.data.bodys && res.data.bodys[CARD.table_code_parent]) {
                                    props.cardTable.setTableData(CARD.table_code, res.data.bodys[CARD.table_code_parent]);
                                }
                            }
...
//弹窗修改
export function windowUpdate(props) {
...
export function cardDelete(props) {
    ...
    ajax({
        ...
        success: (res) => {
            if (res.success) {
                toast({ color: STATUS.success, content: props.json['81H10105-000005'] });/* 国际化处理： 删除成功*/
                // 获取下一条数据的id
...
export function getCardData(props, id, isRefresh = false, callBack) {
    ...
    if (!isRefresh) {
        ...
        if (cardData) {//有缓存
            ...
            cardData.bodys && props.cardTable.setTableData(CARD.table_code_change, cardData.bodys[CARD.table_code_change]);
            cardData.bodys && props.cardTable.setTableData(CARD.table_code_parent, cardData.bodys[CARD.table_code_parent]);
            setByStatus(props);
            return;
        }
    }
        ...
        success: (res) => {
            ...
            if (success) {
                ...
                if (data && data.bodys) {
                    ...
                    props.cardTable.setTableData(CARD.table_code_change, data.bodys[CARD.table_code_change]);
                    props.cardTable.setTableData(CARD.table_code_parent, data.bodys[CARD.table_code_parent]);
                }
                // 更新缓存
                updateCache(PRIMARY_KEY.head_id, id, data, CARD.form_id, DATASOURCE);
    ...
    ajax({
        ...
        success: (res) => {
            if (res.success) { //成功
                ...
                toast({ color: STATUS.warning, content: props.json['81H10105-000015'] });/* 国际化处理： 提交失败*/
            }
        }
    });
}
    ...
    ajax({
        ...
        success: (res) => {
            if (res.success) { //成功
                ...
                toast({ color: STATUS.warning, content: props.json['81H10105-000017'] });/* 国际化处理： 收回失败*/
            }
        }
    });
}
