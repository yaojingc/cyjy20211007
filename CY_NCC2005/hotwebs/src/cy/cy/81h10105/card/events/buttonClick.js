...
export function buttonClick(props, id, hotkey, isFromSide) {
    switch (id) {
        //学生期初录入
        case CARD_BUTTON.beginSave:
            props.modal.show(CARD.table_code, {
                content: <StuBegin {...props} {...{
                    json: this.state.json,
                    actionName: id
                }} />,
                // closeModalEve: () => {
                //     props.editTable.getStatus('card_body') !== 'browse' || props.modal.close('card_body');
                // }
            });
            break;
        ...
        case CARD_BUTTON.cancel:
            promptBox({
                color: STATUS.warning,
                title: this.state.json['81H10105-000002'],/* 国际化处理： 取消*/
                content: this.state.json['81H10105-000003'],/* 国际化处理： 是否确认要取消?*/
                beSureBtnClick: () => {
