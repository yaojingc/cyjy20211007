...
function modifierMeta(props, meta) {
        ...
        meta[CARD.form_id].items.map(item => {
                ...
                if (item.attrcode === 'saleman' || item.attrcode === 'senior' || item.attrcode === 'def13' || item.attrcode === 'def14' || item.attrcode === 'def16') {
                        item.queryCondition = () => {
                ...
                if (item.attrcode === 'province') {
                        // item.disabled = !0,
                        item.queryCondition = () => {
                                let country = props.form.getFormItemsValue(CARD.form_id, 'country').value;
                                return {
                                        pk_country: country,
                                        pk_father: '~'
                                };
                ...
                if (item.attrcode === 'city') {
                        // item.disabled = !0,
                        item.queryCondition = () => {
                                let province = props.form.getFormItemsValue(CARD.form_id, 'province').value;
                                return {
                                        pk_father: province
                                };
                        };
                }
                if (item.attrcode === 'county') {
                        // item.disabled = !0,
                        item.queryCondition = () => {
                                let city = props.form.getFormItemsValue(CARD.form_id, 'city').value;
                                return {
                                        pk_father: city
                                };
                        };
        ...
        // 表体人员参照
        meta[CARD.table_code_busin].items.map(item => {// 人员
                if (item.attrcode === 'def1' || item.attrcode === 'def2' || item.attrcode === 'def3' || item.attrcode === 'def4') {
                        item.queryCondition = () => {
                                let pkOrgsValue = props.form.getFormItemsValue(CARD.form_id, 'pk_org').value;
                                return {
                                        unitPks: pkOrgsValue,
                                        busifuncode: 'it',
                                };
        ...
        for (let item of Object.keys(meta.gridrelation)) {
                meta[item].items.push({
                        attrcode: 'opr',
                        label: this.state.json['81H10101-000006'],/* 国际化处理： 操作*/
                        itemtype: 'customer',
                        fixed: 'right',
                        className: 'table-opr',
                        visible: true,
