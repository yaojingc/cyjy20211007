...
export function cardSave(props) {
    return new Promise((resolve, rejected) => {
        ...
        if (flag) {
            ...
            debugger
            let data = props.createExtCardData(CARD.page_code, CARD.form_id, [CARD.table_code, CARD.table_code_link, CARD.table_code_class, CARD.table_code_top, CARD.table_code_busin]);
            props.validateToSave(data, () => {
                ...
                ajax({
                    ...
                    success: (res) => {
                        ...
                        if (success) {
                            toast({ color: STATUS.success, content: props.json['81H10101-000004'] });/* 国际化处理： 保存成功*/
                            ...
                            if (res.data) {
                                ...
                                if (res.data.bodys && res.data.bodys[CARD.table_code]) {
                                    props.cardTable.setTableData(CARD.table_code, res.data.bodys[CARD.table_code]);
                                }
                                if (res.data.bodys && res.data.bodys[CARD.table_code_link]) {
                                    props.cardTable.setTableData(CARD.table_code_link, res.data.bodys[CARD.table_code_link]);
                                }
                                if (res.data.bodys && res.data.bodys[CARD.table_code_class]) {
                                    props.cardTable.setTableData(CARD.table_code_class, res.data.bodys[CARD.table_code_class]);
                                }
                                if (res.data.bodys && res.data.bodys[CARD.table_code_top]) {
                                    props.cardTable.setTableData(CARD.table_code_top, res.data.bodys[CARD.table_code_top]);
                                }
                                if (res.data.bodys && res.data.bodys[CARD.table_code_busin]) {
                                    props.cardTable.setTableData(CARD.table_code_busin, res.data.bodys[CARD.table_code_busin]);
                                }
                            }
...
export function getCardData(props, id, isRefresh = false, callBack) {
    ...
    if (!isRefresh) {
        ...
        if (cardData) {//有缓存
            ...
            cardData.bodys && props.cardTable.setTableData(CARD.table_code, cardData.bodys[CARD.table_code]);
            cardData.bodys && props.cardTable.setTableData(CARD.table_code_link, cardData.bodys[CARD.table_code_link]);
            cardData.bodys && props.cardTable.setTableData(CARD.table_code_class, cardData.bodys[CARD.table_code_class]);
            cardData.bodys && props.cardTable.setTableData(CARD.table_code_top, cardData.bodys[CARD.table_code_top]);
            cardData.bodys && props.cardTable.setTableData(CARD.table_code_busin, cardData.bodys[CARD.table_code_busin]);
            setByStatus(props);
            return;
        }
    }
        ...
        success: (res) => {
            ...
            if (success) {
                ...
                if (data && data.bodys) {
                    ...
                    props.cardTable.setTableData(CARD.table_code_link, data.bodys[CARD.table_code_link]);
                    props.cardTable.setTableData(CARD.table_code_class, data.bodys[CARD.table_code_class]);
                    props.cardTable.setTableData(CARD.table_code_top, data.bodys[CARD.table_code_top]);
                    props.cardTable.setTableData(CARD.table_code_busin, data.bodys[CARD.table_code_busin]);
                }
                // 更新缓存
                updateCache(PRIMARY_KEY.head_id, id, data, CARD.form_id, DATASOURCE);
