...
export function bodyButtonClick(props, key, text, record, index) {
    ...
    switch (key) {
        ...
        //肩 新增（联系人信息）
        case CARD_BUTTON.addRowLink:
            props.cardTable.addRow(CARD.table_code_link);
            break;

        //肩 删除（联系人信息）
        case CARD_BUTTON.deleteRowLink:
            let checkedRowsLink = props.cardTable.getCheckedRows(CARD.table_code_link);
            let checkedIndexLink = checkedRowsLink && checkedRowsLink.map(item => item.index);
            if (checkedRowsLink.length > 0) {
                props.cardTable.delRowsByIndex(CARD.table_code_link, checkedIndexLink);
                props.button.setButtonDisabled(CARD_DISABLED_BUTTON, true);
            }
            break;
         
        //肩 新增（年纪情况）
        case CARD_BUTTON.addRowClass:
            props.cardTable.addRow(CARD.table_code_class);
            break;

        //肩 删除（年纪情况）
        case CARD_BUTTON.deleteRowClass:
            let checkedRowsClass = props.cardTable.getCheckedRows(CARD.table_code_class);
            let checkedIndexClass = checkedRowsClass && checkedRowsClass.map(item => item.index);
            if (checkedRowsClass.length > 0) {
                props.cardTable.delRowsByIndex(CARD.table_code_class, checkedIndexClass);
                props.button.setButtonDisabled(CARD_DISABLED_BUTTON, true);
            }
            break;
            
        //肩 新增（高管维护记录）
        case CARD_BUTTON.addRowTop:
            props.cardTable.addRow(CARD.table_code_top);
            break;

        //肩 删除（高管维护记录）
        case CARD_BUTTON.deleteRowTop:
            let checkedRowsTop = props.cardTable.getCheckedRows(CARD.table_code_top);
            let checkedIndexTop = checkedRowsTop && checkedRowsTop.map(item => item.index);
            if (checkedRowsTop.length > 0) {
                props.cardTable.delRowsByIndex(CARD.table_code_top, checkedIndexTop);
                props.button.setButtonDisabled(CARD_DISABLED_BUTTON, true);
            }
            break;
            
        //肩 新增（学校业务变动）
        case CARD_BUTTON.addRowBusin:
            props.cardTable.addRow(CARD.table_code_busin);
            break;

        //肩 删除（学校业务变动）
        case CARD_BUTTON.deleteRow:
            let checkedRowsBusin = props.cardTable.getCheckedRows(CARD.table_code_busin);
            let checkedIndexBusin = checkedRowsBusin && checkedRowsBusin.map(item => item.index);
            if (checkedRowsBusin.length > 0) {
                props.cardTable.delRowsByIndex(CARD.table_code_busin, checkedIndexBusin);
                props.button.setButtonDisabled(CARD_DISABLED_BUTTON, true);
            }
            break;
