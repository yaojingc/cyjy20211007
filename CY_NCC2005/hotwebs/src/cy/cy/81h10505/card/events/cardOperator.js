            ...
            props.validateToSave(data, () => {
                ...
                ajax({
                    ...
                    success: (res) => {
                        ...
                        if (success) {
                            toast({ color: STATUS.success, content: props.json['81H10505-000004'] });/* 国际化处理： 保存成功*/
...
export function cardBillTrack(props) {
    ...
    let billtypecode = props.form.getFormItemsValue(CARD.form_id, 'transtypecode') && props.form.getFormItemsValue(CARD.form_id, 'transtypecode').value;
    this.setState({
        showBillTrack: true,
        billTrackBillId: billId,
        billTrackBillType: billtypecode
    });
}
