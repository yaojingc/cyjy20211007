...
export function listRefresh(props) {
    console.log("listRefresh:props--", props)
    debugger
...
// 老师讲师取消分配
export function cancelTeacherAssign(props, data) {
    debugger
    if (!data) {
    ...
    ajax({
        url: REQUEST_URL.cancelTeacherAssign,
        data: data,
        success: (res) => {
            if (res.success) { //成功
                listRefresh(props);
            } else { //失败
                toast({ color: STATUS.warning, content: "取消分配失败" });/* 国际化处理： 收回失败*/
            }
        }
    });
}
...
// 老师讲师分配
export function teacherAssign(indexProps, data) {
    if (data) {
        ajax({
            ...
            success: (res) => {
                if (res.success) { //成功
                    ...
                    toast({ color: STATUS.warning, content: "分配失败" });/* 国际化处理： 收回失败*/
                }
            }
        });
    }
    ...
    ajax({
        ...
        success: (res) => {
            if (res.success) { //成功
                ...
                PromptMessage.call(this, res);
                //toast({ color: STATUS.warning, content: props.json['81H10505-000015'] });/* 国际化处理： 提交失败*/
            }
        }
    });
}
    ...
    ajax({
        ...
        success: (res) => {
            if (res.success) { //成功
                ...
                toast({ color: STATUS.warning, content: props.json['81H10505-000017'] });/* 国际化处理： 收回失败*/
            }
        }
    });
}
...
export function checkSelected(props, isCheckOne) {
    ...
    if (isCheckOne && selectDatas.length > 1) {
        toast({
            color: STATUS.warning, content: props.json['81H10505-000012']/* 国际化处理： 请选中一行表体数据!*/
        });
        valid = false;
    } else if (selectDatas.length == 0) {
        toast({
            color: STATUS.warning, content: props.json['81H10505-000013']/* 国际化处理： 请选择表体数据操作!*/
        });
        valid = false;
    }
...
export function listBillTrack(props) {
    ...
    if (select.valid) {
        let selectDatas = select.selectDatas;
        let billId = selectDatas[0] && selectDatas[0].data.values[PRIMARY_KEY.head_id].value;
        let transtypecode = selectDatas[0].data.values['transtypecode'].value; 
        this.setState({
            showBillTrack: true,
            billTrackBillId: billId,
            billTrackBillType: transtypecode
        });
    }
}
    ...
    ajax({
        ...
        success: (res) => {
            if (res.success) { //成功
                ...
                toast({ color: STATUS.success, content: props.json['81H10505-000005'] });/* 国际化处理： 删除成功*/
            } else { //失败
                toast({ color: STATUS.warning, content: props.json['81H10505-000009'] });/* 国际化处理： 删除失败*/
            }
        }
    });
}
    ...
    ajax({
        ...
        success: (res) => {
            ...
            if (success && data && data[LIST.table_id]) {
                ...
                toast({ color: STATUS.warning, content: props.json['81H10505-000010'] });/* 国际化处理： 未查询出符合条件的数据！*/
            }
