    ...
    ajax({
        ...
        success: (res) => {
            if (res.success) { //成功
                ...
                PromptMessage.call(this, res);
                //toast({ color: STATUS.warning, content: props.json['81H10131-000015'] });/* 国际化处理： 提交失败*/
            }
        }
    });
}
    ...
    ajax({
        ...
        success: (res) => {
            if (res.success) { //成功
                ...
                toast({ color: STATUS.warning, content: props.json['81H10131-000017'] });/* 国际化处理： 收回失败*/
            }
        }
    });
}
...
export function checkSelected(props, isCheckOne) {
    ...
    if (isCheckOne && selectDatas.length > 1) {
        toast({
            color: STATUS.warning, content: props.json['81H10131-000012']/* 国际化处理： 请选中一行表体数据!*/
        });
        valid = false;
    } else if (selectDatas.length == 0) {
        toast({
            color: STATUS.warning, content: props.json['81H10131-000013']/* 国际化处理： 请选择表体数据操作!*/
        });
        valid = false;
    }
    ...
    ajax({
        ...
        success: (res) => {
            if (res.success) { //成功
                ...
                toast({ color: STATUS.success, content: props.json['81H10131-000005'] });/* 国际化处理： 删除成功*/
            } else { //失败
                toast({ color: STATUS.warning, content: props.json['81H10131-000009'] });/* 国际化处理： 删除失败*/
            }
        }
    });
}
    ...
    ajax({
        ...
        success: (res) => {
            ...
            if (success && data && data[LIST.table_id]) {
                ...
                toast({ color: STATUS.warning, content: props.json['81H10131-000010'] });/* 国际化处理： 未查询出符合条件的数据！*/
            }
