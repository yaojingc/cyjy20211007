...
// 选中班级后查询班级下的所有学生数据赋值表体
export function getAllStuOfClasscy(props, data) {
    let retData = null;
    if (data) {
        
        ajax({
            url: REQUEST_URL.queryStuOfClasscy,
            data: data,
            success: (res) => {
                ...
                if (success) {
                    if (res.data) {
                        retData = res.data;
                        retData.map(item => {
                            console.log("--item.values.index--", item.values.index);
                            props.cardTable.addRow(CARD.table_code, item.values.index.value,item.values);
                        });
                        ...
                        toast({ color: STATUS.warning, content: "该班级无学生"});
                    }
                }
            }
        });
            ...
            props.validateToSave(data, () => {
                ...
                ajax({
                    ...
                    success: (res) => {
                        let { success, data } = res;
                        if (success) {
                            toast({ color: STATUS.success, content: props.json['81H11025-000004'] });/* 国际化处理： 保存成功*/
...
export function cardDelete(props) {
    ...
    ajax({
        ...
        success: (res) => {
            if (res.success) {
                toast({ color: STATUS.success, content: props.json['81H11025-000005'] });/* 国际化处理： 删除成功*/
    ...
    ajax({
        ...
        success: (res) => {
            if (res.success) { //成功
                ...
                toast({ color: STATUS.warning, content: props.json['81H11025-000015'] });/* 国际化处理： 提交失败*/
            }
        }
    });
}
    ...
    ajax({
        ...
        success: (res) => {
            if (res.success) { //成功
                ...
                toast({ color: STATUS.warning, content: props.json['81H11025-000017'] });/* 国际化处理： 收回失败*/
            }
        }
    });
}
