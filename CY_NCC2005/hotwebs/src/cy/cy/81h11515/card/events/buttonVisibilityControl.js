...
export function buttonVisibilityControl(props) {
   ...
   // props.button.setButtonVisible(CARD_BUTTON.setReturnMoneyBtn);
   //设置设置金额按钮可见性 add by csh 
   //let id = props.getUrlParam(PRIMARY_KEY.id);
   debugger;
   if(id != undefined){
        let params = {
            pk_bill: id,
            checkman: getBusinessInfo().userId
        }
        ajax({
            url: '/nccloud/cy/cy/checKManIsEditReturnMoney.do',
            data: params,
            async: false,
            success: (res) => {
                    debugger;
                    if (res.data) {
                        if(res.data.isEdit=='Y'){
                                props.button.setButtonVisible(CARD_BUTTON.setReturnMoneyBtn, true);
                        }else{
                                props.button.setButtonVisible(CARD_BUTTON.setReturnMoneyBtn, false);
                        }
                    }
            }
        });
