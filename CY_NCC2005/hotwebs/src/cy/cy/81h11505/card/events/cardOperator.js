...
import { CARD_ADD_DISABLED_BUTTON_XFJM,REQUEST_URL, CARD, PRIMARY_KEY, CARD_BUTTON, CARD_CACHE, DATASOURCE, STATUS, FIELD, BILL_TYPE_CODE, CARD_ADD_DISABLED_BUTTON } from '../../constant';
...
export function cardModify(props) {
    ...
    promptBox({
        ...
        beSureBtnClick: () => {
            ...
            copyClear(props);
            setByStatus(props);
            // props.button.setButtonVisible(CARD_BUTTON.save,true);
            // 设置表体列不可编辑，待拓展
            // props.form.setFormItemsVisible(CARD.form_id,{'def7':false})
           
            // props.form.setFormItemsDisabled(CARD.form_id,{'def7':false})

            // props.cardTable.setColEditableByKey(CARD.table_code, 'proplan', false);
            
            // props.cardTable.setAllCheckboxAble(CARD.table_code, false);
            
            // props.cardTable.setAllCheckboxAble(CARD.card_head, false);
            props.button.setButtonDisabled(CARD_ADD_DISABLED_BUTTON_XFJM, true);//恢复增行编辑性
            this.props.form.setFormItemsDisabled(CARD.form_id,{'def7':true})
        }
    });
    ...
    //     return;
    // }
    // if (select.valid && select.selectDatas.length == 1) {
    //     let pk_contract = select.selectDatas[0].data.values.pk_contract;
    ...
    //     });
    // } else {
    //     toast({ color: STATUS.warning, content: '请选择一条记录进行变更' });
    // }
            ...
            props.validateToSave(data, () => {
                ...
                ajax({
                    ...
                    success: (res) => {
                        ...
                        if (success) {
                            toast({ color: STATUS.success, content: props.json['81H11505-000004'] });/* 国际化处理： 保存成功*/
...
export function cardDelete(props) {
    ...
    ajax({
        ...
        success: (res) => {
            if (res.success) {
                toast({ color: STATUS.success, content: props.json['81H11505-000005'] });/* 国际化处理： 删除成功*/
    ...
    ajax({
        ...
        success: (res) => {
            if (res.success) { //成功
                ...
                toast({ color: STATUS.warning, content: props.json['81H11505-000015'] });/* 国际化处理： 提交失败*/
            }
        }
    });
}
    ...
    ajax({
        ...
        success: (res) => {
            if (res.success) { //成功
                ...
                toast({ color: STATUS.warning, content: props.json['81H11505-000017'] });/* 国际化处理： 收回失败*/
            }
        }
    });
}
