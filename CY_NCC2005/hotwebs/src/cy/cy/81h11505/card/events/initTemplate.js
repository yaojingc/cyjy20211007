...
function modifierMeta(props, meta) {
        ...
        meta[CARD.form_id].items.map(item => {
                ...
                //表头学费减免参照过滤
                if(item.attrcode === 'def7'){
                        item.queryCondition = () => {
                                return {
                                        // 学费收款合同中表头的def2 存的学生参照的主键
                                        pk_student: props.form.getFormItemsValue(CARD.form_id, 'def2').value
                                };
                        };
                }
        ...
        for (let item of Object.keys(meta.gridrelation)) {
                meta[item].items.push({
                        attrcode: 'opr',
                        label: this.state.json['81H11505-000006'],/* 国际化处理： 操作*/
                        itemtype: 'customer',
                        fixed: 'right',
                        className: 'table-opr',
                        visible: true,
