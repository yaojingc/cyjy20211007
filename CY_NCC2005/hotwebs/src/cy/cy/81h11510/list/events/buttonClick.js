...
import { invoiceWhether,listCreate, listRefresh, listHeadDelete, listAttachment, listBillTrack, listCommit, listUnCommit, listLinkApprove } from './listOperator';
...
export function buttonClick(props, id, text, record, index) {
        switch (id) {
                 //已开票
                 case LIST_BUTTON.invoice:
                        promptBox({
                                color: STATUS.warning,
                                title: this.state.json['81H11510-000019'],/* 国际化处理： 已开票*/
                                content: this.state.json['81H11510-000020'],/* 国际化处理： 确定已开票吗？*/
                                beSureBtnClick: () => {
                                        invoiceWhether({ ...props, json: this.state.json });
                                }
                        });
                        break;
                ...
                case LIST_BUTTON.delete:
                        promptBox({
                                color: STATUS.warning,
                                title: this.state.json['81H11510-000000'],/* 国际化处理： 删除*/
                                content: this.state.json['81H11510-000001'],/* 国际化处理： 确定删除吗？*/
                                beSureBtnClick: () => {
