...
export function buttonVisibilityControl(props) {
    ...
    */
    ...
    //将要显示的按钮
    let showBtn = [];
    //编辑态显示按钮  CARD_BUTTON.deleteRow,  CARD_BUTTON.delRow,
    let editBtn = [CARD_BUTTON.save, CARD_BUTTON.saveAdd, CARD_BUTTON.saveCommit, CARD_BUTTON.cancel,
    CARD_BUTTON.addRow,  CARD_BUTTON.copyRows,
    CARD_BUTTON.insertRow,  CARD_BUTTON.copyRow, CARD_BUTTON.pasteHere, CARD_BUTTON.expand];
    //联查按钮
    ...
    if (!isBrowse) { //编辑态
        ...
        if (!id) {//点击新增然后点击取消后
            ...
            switch (billStatus) {
                case STATUS.NOSTATE:
                    showBtn = [CARD_BUTTON.update, CARD_BUTTON.commit, ...commonBtn];//CARD_BUTTON.delete,
                    break;
                case STATUS.PASSING:
                case STATUS.COMMIT:
                case STATUS.GOINGON:
