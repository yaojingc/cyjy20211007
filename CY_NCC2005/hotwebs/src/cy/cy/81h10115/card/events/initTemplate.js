...
function modifierMeta(props, meta) {
        ...
        meta[CARD.form_id].items.map(item => {
                ...
                if (item.attrcode === 'teacher'||item.attrcode === 'gangboss'||item.attrcode === 'supervisor'||item.attrcode === 'marketass'
                ||item.attrcode === 'salesman'||item.attrcode === 'dutypsn') { 
                        item.queryCondition = () => {
        ...
        for (let item of Object.keys(meta.gridrelation)) {
                meta[item].items.push({
                        attrcode: 'opr',
                        label: this.state.json['81H10115-000006'],/* 国际化处理： 操作*/
                        itemtype: 'customer',
                        fixed: 'right',
                        className: 'table-opr',
                        visible: true,
