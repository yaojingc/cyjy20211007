            ...
            props.validateToSave(data, () => {
                ...
                ajax({
                    ...
                    success: (res) => {
                        ...
                        if (success) {
                            toast({ color: STATUS.success, content: props.json['81H10535-000004'] });/* 国际化处理： 保存成功*/
                            ...
                            if (res.data) {
                                ...
                                if (res.data.bodys && res.data.bodys[CARD.table_code_sch]) {
                                    props.cardTable.setTableData(CARD.table_code_sch, res.data.bodys[CARD.table_code_sch]);
                                }
                                if (res.data.bodys && res.data.bodys[CARD.table_code_kc]) {
                                    props.cardTable.setTableData(CARD.table_code_kc, res.data.bodys[CARD.table_code_kc]);
                                }
                            }
...
export function cardDelete(props) {
    ...
    ajax({
        ...
        success: (res) => {
            if (res.success) {
                toast({ color: STATUS.success, content: props.json['81H10535-000005'] });/* 国际化处理： 删除成功*/
...
export function getCardData(props, id, isRefresh = false, callBack) {
    ...
    if (!isRefresh) {
        ...
        if (cardData) {//有缓存
            ...
            cardData.bodys && props.cardTable.setTableData(CARD.table_code_sch, cardData.bodys[CARD.table_code_sch]);
            cardData.bodys && props.cardTable.setTableData(CARD.table_code_kc, cardData.bodys[CARD.table_code_kc]);
            setByStatus(props);
            return;
        }
    }
        ...
        success: (res) => {
            ...
            if (success) {
                ...
                if (data && data.bodys) {
                    ...
                    props.cardTable.setTableData(CARD.table_code_sch, data.bodys[CARD.table_code_sch]);
                    props.cardTable.setTableData(CARD.table_code_kc, data.bodys[CARD.table_code_kc]);
                }
                // 更新缓存
                updateCache(PRIMARY_KEY.head_id, id, data, CARD.form_id, DATASOURCE);
    ...
    ajax({
        ...
        success: (res) => {
            if (res.success) { //成功
                ...
                toast({ color: STATUS.warning, content: props.json['81H10535-000015'] });/* 国际化处理： 提交失败*/
            }
        }
    });
}
    ...
    ajax({
        ...
        success: (res) => {
            if (res.success) { //成功
                ...
                toast({ color: STATUS.warning, content: props.json['81H10535-000017'] });/* 国际化处理： 收回失败*/
            }
        }
    });
}
