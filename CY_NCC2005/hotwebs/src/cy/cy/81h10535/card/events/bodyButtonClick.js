...
export function bodyButtonClick(props, key, text, record, index) {
    ...
    switch (key) {
        ...
        //肩 新增（行程安排）
        case CARD_BUTTON.addRowSch:
            props.cardTable.addRow(CARD.table_code_sch);
            break;

        //肩 删除
        case CARD_BUTTON.deleteRowSch:
            let checkedRowsSch = props.cardTable.getCheckedRows(CARD.table_code_sch);
            let checkedIndexSch = checkedRows && checkedRowsSch.map(item => item.index);
            if (checkedRows.length > 0) {
                props.cardTable.delRowsByIndex(CARD.table_code_sch, checkedIndexSch);
                props.button.setButtonDisabled(CARD_DISABLED_BUTTON, true);
            }
            break;

        //肩 新增（考察效果）
        case CARD_BUTTON.addRowKc:
            props.cardTable.addRow(CARD.table_code_kc);
            break;

        //肩 删除
        case CARD_BUTTON.deleteRowKc:
            let checkedRowsKc = props.cardTable.getCheckedRows(CARD.table_code_kc);
            let checkedIndexKc = checkedRows && checkedRowsKc.map(item => item.index);
            if (checkedRows.length > 0) {
                props.cardTable.delRowsByIndex(CARD.table_code_kc, checkedIndexKc);
                props.button.setButtonDisabled(CARD_DISABLED_BUTTON, true);
            }
            break;
