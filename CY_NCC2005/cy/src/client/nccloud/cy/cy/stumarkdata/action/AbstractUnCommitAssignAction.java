...
import nccloud.vo.cy.cy.StumarkdataConst;
...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggStumarkdataHVO[] commitVOs,
  ...
  protected Object callActionScript(AggStumarkdataHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggStumarkdataHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggStumarkdataHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(StumarkdataConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggStumarkdataHVO> bills = service.queryBillOfVOByCond(
              AggStumarkdataHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggStumarkdataHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggStumarkdataHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            StumarkdataConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggStumarkdataHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(StumarkdataConst.CONST_BILL_STATUS);
    ...
    if (hasFlow) {
      result = this.callActionScript(commitVOs,
              StumarkdataConst.CONST_BILLTYPE_COST);
    }
    // 没有，则收回即弃审
    else {
