...
import nccloud.cy.cy.msgstationhvo.cy.bean.PageQueryVO;
 ...
 */
public class MsgstationHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggMsgstationHVO[] bills =
        (AggMsgstationHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggMsgstationHVO[] bills =
        Constructor.construct(AggMsgstationHVO.class, 0);
   ...
   * @return
   */
  private AggMsgstationHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggMsgstationHVO... aggvos) {
    ...
    List<AggMsgstationHVO> billLst = new ArrayList<AggMsgstationHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggMsgstationHVO bill = new AggMsgstationHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggMsgstationHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggMsgstationHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggMsgstationHVO bill : aggvos) {
