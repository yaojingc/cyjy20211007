 ...
 */
public class MsgstationHVOSaveAction implements ICommonAction {
...
AggMsgstationHVO vo = billCardOperator.toBill(paramIRequest);
      this.doBefore(vo);
      // 2、调用单据的保存动作脚本（savebase），得到保存后结果
AggMsgstationHVO rtnObj = this.callActionScript(vo);
  ...
  protected String getActionCode() {
    return MsgstationHVOConst.CONST_ACTION_SAVEBASE;
  }
   ...
   */
  private AggMsgstationHVO callActionScript(AggMsgstationHVO... aggVOs)
      throws BusinessException {

    String actionCode = this.getActionCode();
    String billType = MsgstationHVOConst.CONST_BILLTYPE_COST;
    ...
    String wheresql =
        "pk_msgstation" + "='"
            + ((AggMsgstationHVO) result[0]).getPrimaryKey() + "'";
    ...
    return bills.toArray(new AggMsgstationHVO[0])[0];
  }
   ...
   * @param vo
   */
  private void doBefore(AggMsgstationHVO vo) {
    ...
    if ((parentPk != null) && !"".equals(parentPk)) {
      ...
      if (allchildren != null) {
        for (CircularlyAccessibleValueObject obj : allchildren) {
          obj.setAttributeValue("pk_msgstation", parentPk);
        }
      }
    }
  }
