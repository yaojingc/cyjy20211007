...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggMsgstationHVO[] commitVOs,
  ...
  protected Object callActionScript(AggMsgstationHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggMsgstationHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggMsgstationHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(MsgstationHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggMsgstationHVO> bills = service.queryBillOfVOByCond(
              AggMsgstationHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggMsgstationHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggMsgstationHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            MsgstationHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggMsgstationHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(MsgstationHVOConst.CONST_BILL_STATUS);
