 ...
 */
public class SchoolBasicsHVODeleteAction implements ICommonAction {
...
BillOperatorParam queryParam = this.getQueryParam(paramIRequest);
    try {
      ...
      AggSchoolBasicsHVO[] bills = this.queryBills(queryParam);
      // ts赋值
  ...
  protected String getActionCode() {
    return SchoolBasicsHVOConst.CONST_ACTION_DELETE;
  }
   ...
   */
  private Object callActionScript(AggSchoolBasicsHVO... aggVOs)
      throws BusinessException {
...
BillOperatorParam queryParam =
   ...
   */
  private AggSchoolBasicsHVO[] queryBills(BillOperatorParam queryParam)
      throws MetaDataException {
    // 1、根据参数查询结果
    IMDPersistenceQueryService service = CommonUtil.getMDPersistenceQueryService();
    String wheresql = SQLUtil.buildSqlForIn(
        "pk_school", queryParam.getPks());

    Collection<AggSchoolBasicsHVO> bills =
        service.queryBillOfVOByCond(AggSchoolBasicsHVO.class, wheresql, true,
            false);
    if ((bills == null) || (bills.size() == 0)) {
      return null;
    }
    return bills.toArray(new AggSchoolBasicsHVO[0]);
  }
}
