...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggSchoolBasicsHVO[] commitVOs,
  ...
  protected Object callActionScript(AggSchoolBasicsHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggSchoolBasicsHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggSchoolBasicsHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(SchoolBasicsHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggSchoolBasicsHVO> bills = service.queryBillOfVOByCond(
              AggSchoolBasicsHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggSchoolBasicsHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggSchoolBasicsHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            SchoolBasicsHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggSchoolBasicsHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(SchoolBasicsHVOConst.CONST_BILL_STATUS);
    ...
    if (hasFlow) {
      result = this.callActionScript(commitVOs,
              SchoolBasicsHVOConst.CONST_BILLTYPE_COST);
    }
    // 没有，则收回即弃审
    else {
