...
import nccloud.cy.cy.schoolbasicshvo.cy.bean.PageQueryVO;
 ...
 */
public class SchoolBasicsHVOQueryListAction implements ICommonAction {
	...
	public Object doAction(IRequest paramIRequest) {
		...
		try {
			...
			// CyCommonUtils.getInstance(ICyCommonSqlUtil.class).queryRoleCodeByUserId(userId);
			// 根据当前登录用户查询对应用户组
			...
			if (!ConstantUtil.grouplist.contains(groupcode)) {
				...
				AggSchoolBasicsHVO[] aggvos = CommonUtil.getFinancingcostMaintainService().query(scheme);// 原始查询结果
				List<AggSchoolBasicsHVO> agglist = new ArrayList<AggSchoolBasicsHVO>();
				// 筛选
				if (CyCommonUtils.isNotEmpty(aggvos) && CyCommonUtils.isNotEmpty(psnids)) {
					for (String psnid : psnids) {
						for (AggSchoolBasicsHVO aggSchoolBasicsHVO : aggvos) {
							String pk_saleman = aggSchoolBasicsHVO.getParentVO().getSaleman();
							if (CyCommonUtils.isNotEmpty(pk_saleman) && pk_saleman.equals(psnid)) {
								agglist.add(aggSchoolBasicsHVO);
							}
						}
					}
				}
				AggSchoolBasicsHVO[] aggvos2 = agglist.toArray(new AggSchoolBasicsHVO[agglist.size()]);
				// 查询服务(scheme);
				if ((aggvos2 != null) && (aggvos2.length > 0)) {
					// 3、处理分页
					String[] pks2 = this.getPks(aggvos2);
					AggSchoolBasicsHVO[] bills2 = this.getDefaulePageBill(queryParam, aggvos2);
					page = new PageQueryVO(pks2, bills2);
				} else {
				...
				// 2、调用服务,获取VO信息(平台默认生成方法，有效率问题，最佳实现要改掉)
				AggSchoolBasicsHVO[] aggvos = CommonUtil.getFinancingcostMaintainService().query(scheme);
				// 查询服务(scheme);
				if ((aggvos != null) && (aggvos.length > 0)) {
					// 3、处理分页
					String[] pks = this.getPks(aggvos);
					AggSchoolBasicsHVO[] bills = this.getDefaulePageBill(queryParam, aggvos);
	...
	private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
		AggSchoolBasicsHVO[] bills = (AggSchoolBasicsHVO[]) pagevo.getCurrentPageBills();
	...
	private PageQueryVO createNullPage() {
		...
		AggSchoolBasicsHVO[] bills = Constructor.construct(AggSchoolBasicsHVO.class, 0);
	 ...
	 * @return
	 */
	private AggSchoolBasicsHVO[] getDefaulePageBill(QueryTreeFormatVO info, AggSchoolBasicsHVO... aggvos) {
		...
		List<AggSchoolBasicsHVO> billLst = new ArrayList<AggSchoolBasicsHVO>();
		for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
			AggSchoolBasicsHVO bill = new AggSchoolBasicsHVO();

			bill.setParent(aggvos[i].getParent());
			billLst.add(bill);
		}
		return billLst.toArray(new AggSchoolBasicsHVO[0]);
	}
	 ...
	 * @return
	 */
	private String[] getPks(AggSchoolBasicsHVO... aggvos) {
		List<String> pks = new ArrayList<String>();
		for (AggSchoolBasicsHVO bill : aggvos) {
