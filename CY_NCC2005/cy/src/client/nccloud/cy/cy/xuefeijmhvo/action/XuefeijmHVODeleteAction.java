 ...
 */
public class XuefeijmHVODeleteAction implements ICommonAction {
...
BillOperatorParam queryParam = this.getQueryParam(paramIRequest);
    try {
      ...
      AggXuefeijmHVO[] bills = this.queryBills(queryParam);
      // ts赋值
  ...
  protected String getActionCode() {
    return XuefeijmHVOConst.CONST_ACTION_DELETE;
  }
   ...
   */
  private Object callActionScript(AggXuefeijmHVO... aggVOs)
      throws BusinessException {
...
BillOperatorParam queryParam =
   ...
   */
  private AggXuefeijmHVO[] queryBills(BillOperatorParam queryParam)
      throws MetaDataException {
    // 1、根据参数查询结果
    IMDPersistenceQueryService service = CommonUtil.getMDPersistenceQueryService();
    String wheresql = SQLUtil.buildSqlForIn(
        "pk_xfapply", queryParam.getPks());

    Collection<AggXuefeijmHVO> bills =
        service.queryBillOfVOByCond(AggXuefeijmHVO.class, wheresql, true,
            false);
    if ((bills == null) || (bills.size() == 0)) {
      return null;
    }
    return bills.toArray(new AggXuefeijmHVO[0]);
  }
}
