...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggXuefeijmHVO[] commitVOs,
  ...
  protected Object callActionScript(AggXuefeijmHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggXuefeijmHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggXuefeijmHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(XuefeijmHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggXuefeijmHVO> bills = service.queryBillOfVOByCond(
              AggXuefeijmHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggXuefeijmHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggXuefeijmHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            XuefeijmHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggXuefeijmHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(XuefeijmHVOConst.CONST_BILL_STATUS);
    ...
    if (hasFlow) {
      result = this.callActionScript(commitVOs,
              XuefeijmHVOConst.CONST_BILLTYPE_COST);
    }
    // 没有，则收回即弃审
    else {
