...
import nccloud.cy.cy.xuefeijmhvo.cy.bean.PageQueryVO;
 ...
 */
public class XuefeijmHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggXuefeijmHVO[] bills =
        (AggXuefeijmHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggXuefeijmHVO[] bills =
        Constructor.construct(AggXuefeijmHVO.class, 0);
   ...
   * @return
   */
  private AggXuefeijmHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggXuefeijmHVO... aggvos) {
    ...
    List<AggXuefeijmHVO> billLst = new ArrayList<AggXuefeijmHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggXuefeijmHVO bill = new AggXuefeijmHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggXuefeijmHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggXuefeijmHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggXuefeijmHVO bill : aggvos) {
