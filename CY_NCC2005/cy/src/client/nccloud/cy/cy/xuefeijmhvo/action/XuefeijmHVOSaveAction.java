 ...
 */
public class XuefeijmHVOSaveAction implements ICommonAction {
...
AggXuefeijmHVO rtnObj = this.callActionScript(vo);
  ...
  protected String getActionCode() {
    return XuefeijmHVOConst.CONST_ACTION_SAVEBASE;
  }
   ...
   */
  private AggXuefeijmHVO callActionScript(AggXuefeijmHVO... aggVOs)
      throws BusinessException {

    String actionCode = this.getActionCode();
    String billType = XuefeijmHVOConst.CONST_BILLTYPE_COST;
    ...
    String wheresql =
        "pk_xfapply" + "='"
            + ((AggXuefeijmHVO) result[0]).getPrimaryKey() + "'";
    Collection<AggXuefeijmHVO> bills =
        CommonUtil.getMDPersistenceQueryService().queryBillOfVOByCond(
            AggXuefeijmHVO.class, wheresql, true, false);
    return bills.toArray(new AggXuefeijmHVO[0])[0];
  }
   ...
   * @param vo
   */
  private void doBefore(AggXuefeijmHVO vo) {
    ...
    if ((parentPk != null) && !"".equals(parentPk)) {
      ...
      if (allchildren != null) {
        for (CircularlyAccessibleValueObject obj : allchildren) {
          obj.setAttributeValue("pk_xfapply", parentPk);
        }
      }
    }
  }
