...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggCallcenterHVO[] commitVOs,
  ...
  protected Object callActionScript(AggCallcenterHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggCallcenterHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggCallcenterHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(CallcenterHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggCallcenterHVO> bills = service.queryBillOfVOByCond(
              AggCallcenterHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggCallcenterHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggCallcenterHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            CallcenterHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggCallcenterHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(CallcenterHVOConst.CONST_BILL_STATUS);
    ...
    if (hasFlow) {
      result = this.callActionScript(commitVOs,
              CallcenterHVOConst.CONST_BILLTYPE_COST);
    }
    // 没有，则收回即弃审
    else {
