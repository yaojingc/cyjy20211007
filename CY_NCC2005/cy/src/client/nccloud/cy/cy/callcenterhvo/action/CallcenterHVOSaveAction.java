 ...
 */
public class CallcenterHVOSaveAction implements ICommonAction {
...
AggCallcenterHVO vo = billCardOperator.toBill(paramIRequest);
      this.doBefore(vo);
      // 2、调用单据的保存动作脚本（savebase），得到保存后结果
AggCallcenterHVO rtnObj = this.callActionScript(vo);
  ...
  protected String getActionCode() {
    return CallcenterHVOConst.CONST_ACTION_SAVEBASE;
  }
   ...
   */
  private AggCallcenterHVO callActionScript(AggCallcenterHVO... aggVOs)
      throws BusinessException {
    ...
    String wheresql =
        "pk_callcenter" + "='"
            + ((AggCallcenterHVO) result[0]).getPrimaryKey() + "'";
    ...
    return bills.toArray(new AggCallcenterHVO[0])[0];
  }
   ...
   * @param vo
   */
  private void doBefore(AggCallcenterHVO vo) {
    ...
    if ((parentPk != null) && !"".equals(parentPk)) {
      ...
      if (allchildren != null) {
        for (CircularlyAccessibleValueObject obj : allchildren) {
          obj.setAttributeValue("pk_callcenter", parentPk);
        }
      }
    }
  }
