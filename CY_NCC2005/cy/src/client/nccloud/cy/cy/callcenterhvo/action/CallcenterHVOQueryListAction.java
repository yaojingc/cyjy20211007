...
import nccloud.cy.cy.callcenterhvo.cy.bean.PageQueryVO;
 ...
 */
public class CallcenterHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggCallcenterHVO[] bills =
        (AggCallcenterHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggCallcenterHVO[] bills =
        Constructor.construct(AggCallcenterHVO.class, 0);
   ...
   * @return
   */
  private AggCallcenterHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggCallcenterHVO... aggvos) {
    ...
    List<AggCallcenterHVO> billLst = new ArrayList<AggCallcenterHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggCallcenterHVO bill = new AggCallcenterHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggCallcenterHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggCallcenterHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggCallcenterHVO bill : aggvos) {
