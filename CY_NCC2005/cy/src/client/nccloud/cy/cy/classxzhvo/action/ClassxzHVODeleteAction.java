 ...
 */
public class ClassxzHVODeleteAction implements ICommonAction {
...
BillOperatorParam queryParam = this.getQueryParam(paramIRequest);
    try {
      ...
      AggClassxzHVO[] bills = this.queryBills(queryParam);
      // ts赋值
  ...
  protected String getActionCode() {
    return ClassxzHVOConst.CONST_ACTION_DELETE;
  }
   ...
   */
  private Object callActionScript(AggClassxzHVO... aggVOs)
      throws BusinessException {
...
BillOperatorParam queryParam =
   ...
   */
  private AggClassxzHVO[] queryBills(BillOperatorParam queryParam)
      throws MetaDataException {
    // 1、根据参数查询结果
    IMDPersistenceQueryService service = CommonUtil.getMDPersistenceQueryService();
    String wheresql = SQLUtil.buildSqlForIn(
        "pk_classxz", queryParam.getPks());

    Collection<AggClassxzHVO> bills =
        service.queryBillOfVOByCond(AggClassxzHVO.class, wheresql, true,
            false);
    if ((bills == null) || (bills.size() == 0)) {
      return null;
    }
    return bills.toArray(new AggClassxzHVO[0]);
  }
}
