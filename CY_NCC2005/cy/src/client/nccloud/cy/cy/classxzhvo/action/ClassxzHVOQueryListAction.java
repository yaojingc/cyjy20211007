...
import nccloud.cy.cy.classxzhvo.cy.bean.PageQueryVO;
 ...
 */
public class ClassxzHVOQueryListAction implements ICommonAction {
	...
	public Object doAction(IRequest paramIRequest) {
		...
		try {
			...
			AggClassxzHVO[] aggvos = CommonUtil.getFinancingcostMaintainService().query(scheme);
			// 查询服务(scheme);
			if ((aggvos != null) && (aggvos.length > 0)) {
				// 3、处理分页
				String[] pks = this.getPks(aggvos);
				AggClassxzHVO[] bills = this.getDefaulePageBill(queryParam, aggvos);
	...
	private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
		AggClassxzHVO[] bills = (AggClassxzHVO[]) pagevo.getCurrentPageBills();
	...
	private PageQueryVO createNullPage() {
		...
		AggClassxzHVO[] bills = Constructor.construct(AggClassxzHVO.class, 0);
	 ...
	 * @return
	 */
	private AggClassxzHVO[] getDefaulePageBill(QueryTreeFormatVO info, AggClassxzHVO... aggvos) {
		...
		List<AggClassxzHVO> billLst = new ArrayList<AggClassxzHVO>();
		for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
			AggClassxzHVO bill = new AggClassxzHVO();

			bill.setParent(aggvos[i].getParent());
			billLst.add(bill);
		}
		return billLst.toArray(new AggClassxzHVO[0]);
	}
	 ...
	 * @return
	 */
	private String[] getPks(AggClassxzHVO... aggvos) {
		List<String> pks = new ArrayList<String>();
		for (AggClassxzHVO bill : aggvos) {
