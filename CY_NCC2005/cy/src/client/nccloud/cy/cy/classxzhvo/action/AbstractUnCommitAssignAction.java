...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggClassxzHVO[] commitVOs,
  ...
  protected Object callActionScript(AggClassxzHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggClassxzHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggClassxzHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(ClassxzHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggClassxzHVO> bills = service.queryBillOfVOByCond(
              AggClassxzHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggClassxzHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggClassxzHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            ClassxzHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggClassxzHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(ClassxzHVOConst.CONST_BILL_STATUS);
    ...
    if (hasFlow) {
      result = this.callActionScript(commitVOs,
              ClassxzHVOConst.CONST_BILLTYPE_COST);
    }
    // 没有，则收回即弃审
    else {
