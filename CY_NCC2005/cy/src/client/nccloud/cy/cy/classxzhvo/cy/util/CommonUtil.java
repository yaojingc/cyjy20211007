...
package nccloud.cy.cy.classxzhvo.cy.util;
...
public class CommonUtil {
	...
	private static IClassxzhvoMaintain maintain = null;
	 ...
	 * @return
	 */
	public static Grid displayGridManager(Grid grid, AggClassxzHVO[] bills) {
		if (null == grid) {
			return grid;
		}
		Set<String> setdef1 = new HashSet<String>();
		for (AggClassxzHVO aggvo : bills) {
			ClassxzHVO hvo = aggvo.getParentVO();
			// def1：学校
	 ...
	 * @return
	 */
	public static BillCard displayCardManager(BillCard billCard, AggClassxzHVO bill) {
		ClassxzHVO hvo = bill.getParentVO();
		// def1：学校
		Set<String> setdef1 = new HashSet<String>();
		String def1 = hvo.getDef1();
		if (MMValueCheck.isNotEmpty(def1)) {
	...
	public static String getBillTypeCode() {
		return ClassxzHVOConst.CONST_BILLTYPE_COST;
	}
	 ...
	 * @return
	 */
	public static synchronized IClassxzhvoMaintain getFinancingcostMaintainService() {
		if (CommonUtil.maintain == null) {
			CommonUtil.maintain = ServiceLocator.find(IClassxzhvoMaintain.class);
		}
	...
	public static void setBillsTs(Map<String, String> pkMapTs, AbstractBill[] bills) {
		...
		for (AbstractBill bill : bills) {
			...
			bill.getParent().setAttributeValue(ClassxzHVOConst.CONST_TS, ts);
		}
	}
