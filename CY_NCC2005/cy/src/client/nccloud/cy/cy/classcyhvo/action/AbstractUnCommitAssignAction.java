...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggClasscyHVO[] commitVOs,
  ...
  protected Object callActionScript(AggClasscyHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggClasscyHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggClasscyHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(ClasscyHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggClasscyHVO> bills = service.queryBillOfVOByCond(
              AggClasscyHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggClasscyHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggClasscyHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            ClasscyHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggClasscyHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(ClasscyHVOConst.CONST_BILL_STATUS);
