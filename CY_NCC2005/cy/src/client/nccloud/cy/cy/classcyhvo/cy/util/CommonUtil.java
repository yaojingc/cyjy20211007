...
package nccloud.cy.cy.classcyhvo.cy.util;
...
public class CommonUtil {
	...
	private static IClasscyhvoMaintain maintain = null;
	 ...
	 * @return
	 */
	public static Grid displayGridManager(Grid grid, AggClasscyHVO[] bills) {
		if (null == grid) {
			return grid;
		}
		Set<String> setdef1 = new HashSet<String>();
		for (AggClasscyHVO aggvo : bills) {
			ClasscyHVO chvo = aggvo.getParentVO();
			// def1：学校
	 ...
	 * @return
	 */
	public static BillCard displayCardManager(BillCard billCard, AggClasscyHVO bill) {
		ClasscyHVO hvo = bill.getParentVO();
		// def1：学校
		Set<String> setdef1 = new HashSet<String>();
	...
	public static String getBillTypeCode() {
		return ClasscyHVOConst.CONST_BILLTYPE_COST;
	}
	 ...
	 * @return
	 */
	public static synchronized IClasscyhvoMaintain getFinancingcostMaintainService() {
		if (CommonUtil.maintain == null) {
			CommonUtil.maintain = ServiceLocator.find(IClasscyhvoMaintain.class);
		}
	...
	public static void setBillsTs(Map<String, String> pkMapTs, AbstractBill[] bills) {
		...
		for (AbstractBill bill : bills) {
			...
			bill.getParent().setAttributeValue(ClasscyHVOConst.CONST_TS, ts);
		}
	}
