...
import nccloud.vo.cy.cy.SalesmanfileHVOConst;
...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggSalesmanfileHVO[] commitVOs,
  ...
  protected Object callActionScript(AggSalesmanfileHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggSalesmanfileHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggSalesmanfileHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(SalesmanfileHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggSalesmanfileHVO> bills = service.queryBillOfVOByCond(
              AggSalesmanfileHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggSalesmanfileHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggSalesmanfileHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            SalesmanfileHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggSalesmanfileHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(SalesmanfileHVOConst.CONST_BILL_STATUS);
    ...
    if (hasFlow) {
      result = this.callActionScript(commitVOs,
              SalesmanfileHVOConst.CONST_BILLTYPE_COST);
    }
    // 没有，则收回即弃审
    else {
