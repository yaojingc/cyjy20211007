 ...
 */
public class SalesmanfileHVOSaveAction implements ICommonAction {
...
AggSalesmanfileHVO vo = billCardOperator.toBill(paramIRequest);
      this.doBefore(vo);
      // 2、调用单据的保存动作脚本（savebase），得到保存后结果
AggSalesmanfileHVO rtnObj = this.callActionScript(vo);
  ...
  protected String getActionCode() {
    return SalesmanfileHVOConst.CONST_ACTION_SAVEBASE;
  }
   ...
   */
  private AggSalesmanfileHVO callActionScript(AggSalesmanfileHVO... aggVOs)
      throws BusinessException {

    String actionCode = this.getActionCode();
    String billType = SalesmanfileHVOConst.CONST_BILLTYPE_COST;
    ...
    String wheresql =
        "pk_salesman" + "='"
            + ((AggSalesmanfileHVO) result[0]).getPrimaryKey() + "'";
    ...
    return bills.toArray(new AggSalesmanfileHVO[0])[0];
  }
   ...
   * @param vo
   */
  private void doBefore(AggSalesmanfileHVO vo) {
    ...
    if ((parentPk != null) && !"".equals(parentPk)) {
      ...
      if (allchildren != null) {
        for (CircularlyAccessibleValueObject obj : allchildren) {
          obj.setAttributeValue("pk_salesman", parentPk);
        }
      }
    }
  }
