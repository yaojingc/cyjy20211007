...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggTaskfeedbackHVO[] commitVOs,
  ...
  protected Object callActionScript(AggTaskfeedbackHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggTaskfeedbackHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggTaskfeedbackHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(TaskfeedbackHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggTaskfeedbackHVO> bills = service.queryBillOfVOByCond(
              AggTaskfeedbackHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggTaskfeedbackHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggTaskfeedbackHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            TaskfeedbackHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggTaskfeedbackHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(TaskfeedbackHVOConst.CONST_BILL_STATUS);
