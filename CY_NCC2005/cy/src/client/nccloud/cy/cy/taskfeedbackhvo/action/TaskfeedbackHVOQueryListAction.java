...
import nccloud.cy.cy.taskfeedbackhvo.cy.bean.PageQueryVO;
 ...
 */
public class TaskfeedbackHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggTaskfeedbackHVO[] bills =
        (AggTaskfeedbackHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggTaskfeedbackHVO[] bills =
        Constructor.construct(AggTaskfeedbackHVO.class, 0);
   ...
   * @return
   */
  private AggTaskfeedbackHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggTaskfeedbackHVO... aggvos) {
    ...
    List<AggTaskfeedbackHVO> billLst = new ArrayList<AggTaskfeedbackHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggTaskfeedbackHVO bill = new AggTaskfeedbackHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggTaskfeedbackHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggTaskfeedbackHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggTaskfeedbackHVO bill : aggvos) {
