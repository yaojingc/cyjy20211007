 ...
 */
public class TaskfeedbackHVOSaveAction implements ICommonAction {
...
AggTaskfeedbackHVO vo = billCardOperator.toBill(paramIRequest);
      this.doBefore(vo);
      // 2、调用单据的保存动作脚本（savebase），得到保存后结果
AggTaskfeedbackHVO rtnObj = this.callActionScript(vo);
  ...
  protected String getActionCode() {
    return TaskfeedbackHVOConst.CONST_ACTION_SAVEBASE;
  }
   ...
   */
  private AggTaskfeedbackHVO callActionScript(AggTaskfeedbackHVO... aggVOs)
      throws BusinessException {

    String actionCode = this.getActionCode();
    String billType = TaskfeedbackHVOConst.CONST_BILLTYPE_COST;
    ...
    String wheresql =
        "pk_taskfeedback" + "='"
            + ((AggTaskfeedbackHVO) result[0]).getPrimaryKey() + "'";
    ...
    return bills.toArray(new AggTaskfeedbackHVO[0])[0];
  }
   ...
   * @param vo
   */
  private void doBefore(AggTaskfeedbackHVO vo) {
    ...
    if ((parentPk != null) && !"".equals(parentPk)) {
      ...
      if (allchildren != null) {
        for (CircularlyAccessibleValueObject obj : allchildren) {
          obj.setAttributeValue("pk_taskfeedback", parentPk);
        }
      }
    }
  }
