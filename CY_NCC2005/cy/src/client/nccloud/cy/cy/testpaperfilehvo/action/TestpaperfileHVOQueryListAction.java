...
import nccloud.cy.cy.testpaperfilehvo.cy.bean.PageQueryVO;
 ...
 */
public class TestpaperfileHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggTestpaperfileHVO[] bills =
        (AggTestpaperfileHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggTestpaperfileHVO[] bills =
        Constructor.construct(AggTestpaperfileHVO.class, 0);
   ...
   * @return
   */
  private AggTestpaperfileHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggTestpaperfileHVO... aggvos) {
    ...
    List<AggTestpaperfileHVO> billLst = new ArrayList<AggTestpaperfileHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggTestpaperfileHVO bill = new AggTestpaperfileHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggTestpaperfileHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggTestpaperfileHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggTestpaperfileHVO bill : aggvos) {
