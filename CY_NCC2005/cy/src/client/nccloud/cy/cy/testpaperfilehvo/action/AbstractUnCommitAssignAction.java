...
import nccloud.vo.cy.cy.TestpaperfileHVOConst;
...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggTestpaperfileHVO[] commitVOs,
  ...
  protected Object callActionScript(AggTestpaperfileHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggTestpaperfileHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggTestpaperfileHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(TestpaperfileHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggTestpaperfileHVO> bills = service.queryBillOfVOByCond(
              AggTestpaperfileHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggTestpaperfileHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggTestpaperfileHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            TestpaperfileHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggTestpaperfileHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(TestpaperfileHVOConst.CONST_BILL_STATUS);
    ...
    if (hasFlow) {
      result = this.callActionScript(commitVOs,
              TestpaperfileHVOConst.CONST_BILLTYPE_COST);
    }
    // 没有，则收回即弃审
    else {
