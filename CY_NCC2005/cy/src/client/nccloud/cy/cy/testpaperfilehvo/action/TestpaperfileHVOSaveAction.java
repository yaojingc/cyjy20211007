 ...
 */
public class TestpaperfileHVOSaveAction implements ICommonAction {
...
AggTestpaperfileHVO vo = billCardOperator.toBill(paramIRequest);
      this.doBefore(vo);
      // 2、调用单据的保存动作脚本（savebase），得到保存后结果
AggTestpaperfileHVO rtnObj = this.callActionScript(vo);
  ...
  protected String getActionCode() {
    return TestpaperfileHVOConst.CONST_ACTION_SAVEBASE;
  }
   ...
   */
  private AggTestpaperfileHVO callActionScript(AggTestpaperfileHVO... aggVOs)
      throws BusinessException {

    String actionCode = this.getActionCode();
    String billType = TestpaperfileHVOConst.CONST_BILLTYPE_COST;
    ...
    String wheresql =
        "pk_testpaperfile" + "='"
            + ((AggTestpaperfileHVO) result[0]).getPrimaryKey() + "'";
    ...
    return bills.toArray(new AggTestpaperfileHVO[0])[0];
  }
   ...
   * @param vo
   */
  private void doBefore(AggTestpaperfileHVO vo) {
    ...
    if ((parentPk != null) && !"".equals(parentPk)) {
      ...
      if (allchildren != null) {
        for (CircularlyAccessibleValueObject obj : allchildren) {
          obj.setAttributeValue("pk_testpaperfile", parentPk);
        }
      }
    }
  }
