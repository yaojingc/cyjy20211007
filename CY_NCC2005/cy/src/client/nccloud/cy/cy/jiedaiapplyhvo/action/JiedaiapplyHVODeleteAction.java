 ...
 */
public class JiedaiapplyHVODeleteAction implements ICommonAction {
...
BillOperatorParam queryParam = this.getQueryParam(paramIRequest);
    try {
      ...
      AggJiedaiapplyHVO[] bills = this.queryBills(queryParam);
      // ts赋值
  ...
  protected String getActionCode() {
    return JiedaiapplyHVOConst.CONST_ACTION_DELETE;
  }
   ...
   */
  private Object callActionScript(AggJiedaiapplyHVO... aggVOs)
      throws BusinessException {
...
BillOperatorParam queryParam =
   ...
   */
  private AggJiedaiapplyHVO[] queryBills(BillOperatorParam queryParam)
      throws MetaDataException {
    // 1、根据参数查询结果
    IMDPersistenceQueryService service = CommonUtil.getMDPersistenceQueryService();
    String wheresql = SQLUtil.buildSqlForIn(
        "pk_jiedaiapply", queryParam.getPks());

    Collection<AggJiedaiapplyHVO> bills =
        service.queryBillOfVOByCond(AggJiedaiapplyHVO.class, wheresql, true,
            false);
    if ((bills == null) || (bills.size() == 0)) {
      return null;
    }
    return bills.toArray(new AggJiedaiapplyHVO[0]);
  }
}
