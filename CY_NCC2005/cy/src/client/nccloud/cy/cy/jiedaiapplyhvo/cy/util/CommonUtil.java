...
package nccloud.cy.cy.jiedaiapplyhvo.cy.util;
...
public class CommonUtil {
	...
	private static IJiedaiapplyhvoMaintain maintain = null;
	...
	public static String getBillTypeCode() {
		return JiedaiapplyHVOConst.CONST_BILLTYPE_COST;
	}
	 ...
	 * @return
	 */
	public static synchronized IJiedaiapplyhvoMaintain getFinancingcostMaintainService() {
		if (CommonUtil.maintain == null) {
			CommonUtil.maintain = ServiceLocator.find(IJiedaiapplyhvoMaintain.class);
		}
	...
	public static void setBillsTs(Map<String, String> pkMapTs, AbstractBill[] bills) {
		...
		for (AbstractBill bill : bills) {
			...
			bill.getParent().setAttributeValue(JiedaiapplyHVOConst.CONST_TS, ts);
		}
	}
	 ...
	 * @return
	 */
	public static ExtBillCard displayCardManager(ExtBillCard billCard, AggJiedaiapplyHVO bill) {
		JiedaiapplyHVO hvo = bill.getParentVO();
		// def1：学校
	 ...
	 * @return
	 */
	public static Grid displayGridManager(Grid grid, AggJiedaiapplyHVO[] bills) {
		if (null == grid) {
			return grid;
		}
		Set<String> setdef1 = new HashSet<String>();
		for (AggJiedaiapplyHVO aggvo : bills) {
			JiedaiapplyHVO chvo = aggvo.getParentVO();
			// def1：学校
