...
import nccloud.cy.cy.jiedaiapplyhvo.cy.bean.PageQueryVO;
 ...
 */
public class JiedaiapplyHVOQueryListAction implements ICommonAction {
	...
	public Object doAction(IRequest paramIRequest) {
		...
		try {
			...
			AggJiedaiapplyHVO[] aggvos = CommonUtil.getFinancingcostMaintainService().query(scheme);
			// 查询服务(scheme);
			if ((aggvos != null) && (aggvos.length > 0)) {
				// 3、处理分页
				String[] pks = this.getPks(aggvos);
				AggJiedaiapplyHVO[] bills = this.getDefaulePageBill(queryParam, aggvos);
	...
	private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
		AggJiedaiapplyHVO[] bills = (AggJiedaiapplyHVO[]) pagevo.getCurrentPageBills();
	...
	private PageQueryVO createNullPage() {
		...
		AggJiedaiapplyHVO[] bills = Constructor.construct(AggJiedaiapplyHVO.class, 0);
	 ...
	 * @return
	 */
	private AggJiedaiapplyHVO[] getDefaulePageBill(QueryTreeFormatVO info, AggJiedaiapplyHVO... aggvos) {
		...
		List<AggJiedaiapplyHVO> billLst = new ArrayList<AggJiedaiapplyHVO>();
		for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
			AggJiedaiapplyHVO bill = new AggJiedaiapplyHVO();

			bill.setParent(aggvos[i].getParent());
			billLst.add(bill);
		}
		return billLst.toArray(new AggJiedaiapplyHVO[0]);
	}
	 ...
	 * @return
	 */
	private String[] getPks(AggJiedaiapplyHVO... aggvos) {
		List<String> pks = new ArrayList<String>();
		for (AggJiedaiapplyHVO bill : aggvos) {
