 ...
 */
public class WorkplanHVOSaveAction implements ICommonAction {
...
AggWorkplanHVO vo = billCardOperator.toBill(paramIRequest);
      this.doBefore(vo);
      // 2、调用单据的保存动作脚本（savebase），得到保存后结果
AggWorkplanHVO rtnObj = this.callActionScript(vo);
...
//      Object billcard = billCardOperator.toCard(rtnObj);
  ...
  protected String getActionCode() {
    return WorkplanHVOConst.CONST_ACTION_SAVEBASE;
  }
   ...
   */
  private AggWorkplanHVO callActionScript(AggWorkplanHVO... aggVOs)
      throws BusinessException {
    ...
    String wheresql =
        "pk_workplan" + "='"
            + ((AggWorkplanHVO) result[0]).getPrimaryKey() + "'";
    ...
    return bills.toArray(new AggWorkplanHVO[0])[0];
  }
   ...
   * @param vo
   */
  private void doBefore(AggWorkplanHVO vo) {
    ...
    if ((parentPk != null) && !"".equals(parentPk)) {
      ...
      if (allchildren != null) {
        for (CircularlyAccessibleValueObject obj : allchildren) {
          obj.setAttributeValue("pk_workplan", parentPk);
        }
      }
    }
  }
