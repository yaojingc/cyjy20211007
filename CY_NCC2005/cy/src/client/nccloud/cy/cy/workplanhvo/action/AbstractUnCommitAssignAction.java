...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggWorkplanHVO[] commitVOs,
  ...
  protected Object callActionScript(AggWorkplanHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggWorkplanHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggWorkplanHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(WorkplanHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggWorkplanHVO> bills = service.queryBillOfVOByCond(
              AggWorkplanHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggWorkplanHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggWorkplanHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            WorkplanHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggWorkplanHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(WorkplanHVOConst.CONST_BILL_STATUS);
    ...
    if (hasFlow) {
      result = this.callActionScript(commitVOs,
              WorkplanHVOConst.CONST_BILLTYPE_COST);
    }
    // 没有，则收回即弃审
    else {
