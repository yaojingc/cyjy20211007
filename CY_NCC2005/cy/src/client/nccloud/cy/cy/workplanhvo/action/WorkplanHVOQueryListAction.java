...
import nccloud.cy.cy.workplanhvo.cy.bean.PageQueryVO;
 ...
 */
public class WorkplanHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggWorkplanHVO[] bills =
        (AggWorkplanHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggWorkplanHVO[] bills =
        Constructor.construct(AggWorkplanHVO.class, 0);
   ...
   * @return
   */
  private AggWorkplanHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggWorkplanHVO... aggvos) {
    ...
    List<AggWorkplanHVO> billLst = new ArrayList<AggWorkplanHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggWorkplanHVO bill = new AggWorkplanHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggWorkplanHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggWorkplanHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggWorkplanHVO bill : aggvos) {
