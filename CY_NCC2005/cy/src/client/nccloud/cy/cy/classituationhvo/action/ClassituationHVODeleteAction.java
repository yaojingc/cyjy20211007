 ...
 */
public class ClassituationHVODeleteAction implements ICommonAction {
...
BillOperatorParam queryParam = this.getQueryParam(paramIRequest);
    try {
      ...
      AggClassituationHVO[] bills = this.queryBills(queryParam);
      // ts赋值
  ...
  protected String getActionCode() {
    return ClassituationHVOConst.CONST_ACTION_DELETE;
  }
   ...
   */
  private Object callActionScript(AggClassituationHVO... aggVOs)
      throws BusinessException {
...
BillOperatorParam queryParam =
   ...
   */
  private AggClassituationHVO[] queryBills(BillOperatorParam queryParam)
      throws MetaDataException {
    // 1、根据参数查询结果
    IMDPersistenceQueryService service = CommonUtil.getMDPersistenceQueryService();
    String wheresql = SQLUtil.buildSqlForIn(
        "pk_situation", queryParam.getPks());

    Collection<AggClassituationHVO> bills =
        service.queryBillOfVOByCond(AggClassituationHVO.class, wheresql, true,
            false);
    if ((bills == null) || (bills.size() == 0)) {
      return null;
    }
    return bills.toArray(new AggClassituationHVO[0]);
  }
}
