...
import nccloud.cy.cy.classituationhvo.cy.bean.PageQueryVO;
 ...
 */
public class ClassituationHVOQueryListAction implements ICommonAction {
	...
	public Object doAction(IRequest paramIRequest) {
		...
		try {
			...
			AggClassituationHVO[] aggvos = CommonUtil.getFinancingcostMaintainService().query(scheme);
			// 查询服务(scheme);
			if ((aggvos != null) && (aggvos.length > 0)) {
				// 3、处理分页
				String[] pks = this.getPks(aggvos);
				AggClassituationHVO[] bills = this.getDefaulePageBill(queryParam, aggvos);
	...
	private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
		AggClassituationHVO[] bills = (AggClassituationHVO[]) pagevo.getCurrentPageBills();
	...
	private PageQueryVO createNullPage() {
		...
		AggClassituationHVO[] bills = Constructor.construct(AggClassituationHVO.class, 0);
	 ...
	 * @return
	 */
	private AggClassituationHVO[] getDefaulePageBill(QueryTreeFormatVO info, AggClassituationHVO... aggvos) {
		...
		List<AggClassituationHVO> billLst = new ArrayList<AggClassituationHVO>();
		for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
			AggClassituationHVO bill = new AggClassituationHVO();

			bill.setParent(aggvos[i].getParent());
			billLst.add(bill);
		}
		return billLst.toArray(new AggClassituationHVO[0]);
	}
	 ...
	 * @return
	 */
	private String[] getPks(AggClassituationHVO... aggvos) {
		List<String> pks = new ArrayList<String>();
		for (AggClassituationHVO bill : aggvos) {
