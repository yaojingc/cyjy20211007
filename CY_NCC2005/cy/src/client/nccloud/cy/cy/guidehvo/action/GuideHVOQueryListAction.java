...
import nccloud.cy.cy.guidehvo.cy.bean.PageQueryVO;
 ...
 */
public class GuideHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggGuideHVO[] bills =
        (AggGuideHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggGuideHVO[] bills =
        Constructor.construct(AggGuideHVO.class, 0);
   ...
   * @return
   */
  private AggGuideHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggGuideHVO... aggvos) {
    ...
    List<AggGuideHVO> billLst = new ArrayList<AggGuideHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggGuideHVO bill = new AggGuideHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggGuideHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggGuideHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggGuideHVO bill : aggvos) {
