...
import nccloud.vo.cy.cy.GuideHVOConst;
...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggGuideHVO[] commitVOs,
  ...
  protected Object callActionScript(AggGuideHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggGuideHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggGuideHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(GuideHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggGuideHVO> bills = service.queryBillOfVOByCond(
              AggGuideHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggGuideHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggGuideHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            GuideHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggGuideHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(GuideHVOConst.CONST_BILL_STATUS);
    ...
    if (hasFlow) {
      result = this.callActionScript(commitVOs,
              GuideHVOConst.CONST_BILLTYPE_COST);
    }
    // 没有，则收回即弃审
    else {
