 ...
 */
public class GuideHVOSaveAction implements ICommonAction {
...
AggGuideHVO vo = billCardOperator.toBill(paramIRequest);
      this.doBefore(vo);
      // 2、调用单据的保存动作脚本（savebase），得到保存后结果
AggGuideHVO rtnObj = this.callActionScript(vo);
  ...
  protected String getActionCode() {
    return GuideHVOConst.CONST_ACTION_SAVEBASE;
  }
   ...
   */
  private AggGuideHVO callActionScript(AggGuideHVO... aggVOs)
      throws BusinessException {

    String actionCode = this.getActionCode();
    String billType = GuideHVOConst.CONST_BILLTYPE_COST;
    ...
    String wheresql =
        "pk_guide" + "='"
            + ((AggGuideHVO) result[0]).getPrimaryKey() + "'";
    ...
    return bills.toArray(new AggGuideHVO[0])[0];
  }
   ...
   * @param vo
   */
  private void doBefore(AggGuideHVO vo) {
    ...
    if ((parentPk != null) && !"".equals(parentPk)) {
      ...
      if (allchildren != null) {
        for (CircularlyAccessibleValueObject obj : allchildren) {
          obj.setAttributeValue("pk_guide", parentPk);
        }
      }
    }
  }
