 ...
 */
public class TaskallocationHVOSaveAction implements ICommonAction {
...
AggTaskallocationHVO vo = billCardOperator.toBill(paramIRequest);
      this.doBefore(vo);
      // 2、调用单据的保存动作脚本（savebase），得到保存后结果
AggTaskallocationHVO rtnObj = this.callActionScript(vo);
  ...
  protected String getActionCode() {
    return TaskallocationHVOConst.CONST_ACTION_SAVEBASE;
  }
   ...
   */
  private AggTaskallocationHVO callActionScript(AggTaskallocationHVO... aggVOs)
      throws BusinessException {
    ...
    String wheresql =
        "pk_taskallocation" + "='"
            + ((AggTaskallocationHVO) result[0]).getPrimaryKey() + "'";
    ...
    return bills.toArray(new AggTaskallocationHVO[0])[0];
  }
   ...
   * @param vo
   */
  private void doBefore(AggTaskallocationHVO vo) {
    ...
    if ((parentPk != null) && !"".equals(parentPk)) {
      ...
      if (allchildren != null) {
        for (CircularlyAccessibleValueObject obj : allchildren) {
          obj.setAttributeValue("pk_taskallocation", parentPk);
        }
      }
    }
  }
