...
import nccloud.cy.cy.taskallocationhvo.cy.bean.PageQueryVO;
 ...
 */
public class TaskallocationHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggTaskallocationHVO[] bills =
        (AggTaskallocationHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggTaskallocationHVO[] bills =
        Constructor.construct(AggTaskallocationHVO.class, 0);
   ...
   * @return
   */
  private AggTaskallocationHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggTaskallocationHVO... aggvos) {
    ...
    List<AggTaskallocationHVO> billLst = new ArrayList<AggTaskallocationHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggTaskallocationHVO bill = new AggTaskallocationHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggTaskallocationHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggTaskallocationHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggTaskallocationHVO bill : aggvos) {
