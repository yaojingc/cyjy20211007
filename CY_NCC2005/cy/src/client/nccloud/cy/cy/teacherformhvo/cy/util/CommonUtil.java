...
package nccloud.cy.cy.teacherformhvo.cy.util;
...
public class CommonUtil {
	...
	private static ITeacherformhvoMaintain maintain = null;
	 ...
	 * @return
	 */
	public static Grid displayGridManager(Grid grid, AggTeacherformHVO[] bills) {
		if (null == grid) {
			return grid;
		}
		Set<String> setDef1 = new HashSet<String>();
		Set<String> setDef2 = new HashSet<String>();
		for (AggTeacherformHVO aggvo : bills) {
			TeacherformHVO hvo = aggvo.getParentVO();
			// def1：学校 ，def2：班级
	 ...
	 * @return
	 */
	public static BillCard displayCardManager(BillCard billCard, AggTeacherformHVO bill) {
		// def1：学校 ，def2：班级
		Set<String> setDef1 = new HashSet<String>();
		Set<String> setDef2 = new HashSet<String>();
		TeacherformHVO hvo = bill.getParentVO();
		// def1：学校 ，def2：班级
		String def1 = hvo.getDef1();
		if (MMValueCheck.isNotEmpty(def1)) {
			setDef1.add(def1);
	...
	public static String getBillTypeCode() {
		return TeacherformHVOConst.CONST_BILLTYPE_COST;
	}
	 ...
	 * @return
	 */
	public static synchronized ITeacherformhvoMaintain getFinancingcostMaintainService() {
		if (CommonUtil.maintain == null) {
			CommonUtil.maintain = ServiceLocator.find(ITeacherformhvoMaintain.class);
		}
	...
	public static void setBillsTs(Map<String, String> pkMapTs, AbstractBill[] bills) {
		...
		for (AbstractBill bill : bills) {
			...
			bill.getParent().setAttributeValue(TeacherformHVOConst.CONST_TS, ts);
		}
	}
