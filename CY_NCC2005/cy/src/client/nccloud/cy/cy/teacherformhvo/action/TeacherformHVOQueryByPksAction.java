 ...
 */
public class TeacherformHVOQueryByPksAction implements ICommonAction {
  ...
  protected Object transGrid(PageQueryInfo info,
    ...
    int i = 0;
    for (AggTeacherformHVO bill : bills) {
      heads[i++] = bill.getParent();
    }

    GridOperator operator = new GridOperator(info.getPagecode());
    Grid grid = operator.toGrid(heads);
	grid = CommonUtil.displayGridManager(grid, bills.toArray(new AggTeacherformHVO[0]));
   ...
   */
  private Collection<AggTeacherformHVO> queryBills(PageQueryInfo queryParam)
      throws MetaDataException {
    // 1、根据参数查询结果
    IMDPersistenceQueryService service = CommonUtil.getMDPersistenceQueryService();
    String wheresql =
        SQLUtil.buildSqlForIn("pk_teacherform",queryParam.getPks());
    Collection<AggTeacherformHVO> bills =
        service.queryBillOfVOByCond(AggTeacherformHVO.class, wheresql, false,
            false);
    return bills;
  }
}
