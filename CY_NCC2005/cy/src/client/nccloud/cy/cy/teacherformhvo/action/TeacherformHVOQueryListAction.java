...
import nccloud.cy.cy.teacherformhvo.cy.bean.PageQueryVO;
 ...
 */
public class TeacherformHVOQueryListAction implements ICommonAction {
	...
	public Object doAction(IRequest paramIRequest) {
		...
		try {
			...
			AggTeacherformHVO[] aggvos = CommonUtil.getFinancingcostMaintainService().query(scheme);
			// 查询服务(scheme);
			if ((aggvos != null) && (aggvos.length > 0)) {
				// 3、处理分页
				String[] pks = this.getPks(aggvos);
				AggTeacherformHVO[] bills = this.getDefaulePageBill(queryParam, aggvos);
	...
	private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
		AggTeacherformHVO[] bills = (AggTeacherformHVO[]) pagevo.getCurrentPageBills();
	...
	private PageQueryVO createNullPage() {
		...
		AggTeacherformHVO[] bills = Constructor.construct(AggTeacherformHVO.class, 0);
	 ...
	 * @return
	 */
	private AggTeacherformHVO[] getDefaulePageBill(QueryTreeFormatVO info, AggTeacherformHVO... aggvos) {
		...
		List<AggTeacherformHVO> billLst = new ArrayList<AggTeacherformHVO>();
		for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
			AggTeacherformHVO bill = new AggTeacherformHVO();

			bill.setParent(aggvos[i].getParent());
			billLst.add(bill);
		}
		return billLst.toArray(new AggTeacherformHVO[0]);
	}
	 ...
	 * @return
	 */
	private String[] getPks(AggTeacherformHVO... aggvos) {
		List<String> pks = new ArrayList<String>();
		for (AggTeacherformHVO bill : aggvos) {
