...
import nccloud.cy.cy.teacherformhvo.cy.bean.CommitReturnInfo;
import nccloud.cy.cy.teacherformhvo.cy.bean.MultipleResult;
import nccloud.cy.cy.teacherformhvo.cy.bean.SingleResult;
...
public abstract class AbstractCommitAssignAction<T extends AbstractBill> implements ICommonAction {
	...
	protected Object commit(T vo, Object userobj, String billType) throws BusinessException {
		Object result = null;
		TeacherformHVO parentVO = (TeacherformHVO) vo.getParentVO();
		String pk_transtype = parentVO.getPk_transtype();
		// 根据交易类型主键查询交易类型编码rongleia
		String code = CyCommonUtils.getInstance(ICyCommonSqlUtil.class).queryCodeByTransId(pk_transtype);
		// billType = "LSSQ-Cxx-002";
		// 判断是否定义审批流
		boolean hasFlow = NCCFlowUtils.hasApproveflowDef(code, vo);
		// 有，则走提交动作脚本
		if (hasFlow) {
			result = this.callActionScript((T[]) new AbstractBill[] { vo }, userobj, code);
		}
		// 没有，则提交即审批
		else {
			result = this.callAutoApproveActionScript((T[]) new AbstractBill[] { vo }, code);
		}
		return result;
	}
