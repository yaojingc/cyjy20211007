 ...
 */
public class TeacherformHVODeleteAction implements ICommonAction {
...
BillOperatorParam queryParam = this.getQueryParam(paramIRequest);
    try {
      ...
      AggTeacherformHVO[] bills = this.queryBills(queryParam);
      // ts赋值
  ...
  protected String getActionCode() {
    return TeacherformHVOConst.CONST_ACTION_DELETE;
  }
   ...
   */
  private Object callActionScript(AggTeacherformHVO... aggVOs)
      throws BusinessException {
...
BillOperatorParam queryParam =
   ...
   */
  private AggTeacherformHVO[] queryBills(BillOperatorParam queryParam)
      throws MetaDataException {
    // 1、根据参数查询结果
    IMDPersistenceQueryService service = CommonUtil.getMDPersistenceQueryService();
    String wheresql = SQLUtil.buildSqlForIn(
        "pk_teacherform", queryParam.getPks());

    Collection<AggTeacherformHVO> bills =
        service.queryBillOfVOByCond(AggTeacherformHVO.class, wheresql, true,
            false);
    if ((bills == null) || (bills.size() == 0)) {
      return null;
    }
    return bills.toArray(new AggTeacherformHVO[0]);
  }
}
