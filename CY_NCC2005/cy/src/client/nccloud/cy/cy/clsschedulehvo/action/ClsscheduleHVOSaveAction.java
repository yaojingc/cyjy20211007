 ...
 */
public class ClsscheduleHVOSaveAction implements ICommonAction {
	...
	public Object doAction(IRequest paramIRequest) {
		try {
			...
			AggClsscheduleHVO rtnObj = this.callActionScript(vo);
	...
	protected String getActionCode() {
		return ClsscheduleHVOConst.CONST_ACTION_SAVEBASE;
	}
	 ...
	 */
	private AggClsscheduleHVO callActionScript(AggClsscheduleHVO... aggVOs) throws BusinessException {

		String actionCode = this.getActionCode();
		String billType = ClsscheduleHVOConst.CONST_BILLTYPE_COST;
		...
		String wheresql = "pk_clsschedule" + "='" + ((AggClsscheduleHVO) result[0]).getPrimaryKey() + "'";
		Collection<AggClsscheduleHVO> bills = CommonUtil.getMDPersistenceQueryService()
				.queryBillOfVOByCond(AggClsscheduleHVO.class, wheresql, true, false);
		return bills.toArray(new AggClsscheduleHVO[0])[0];
	}
	 ...
	 * @param vo
	 */
	private void doBefore(AggClsscheduleHVO vo) {
		...
		if ((parentPk != null) && !"".equals(parentPk)) {
			...
			if (allchildren != null) {
				for (CircularlyAccessibleValueObject obj : allchildren) {
					obj.setAttributeValue("pk_clsschedule", parentPk);
				}
			}
		}
	}
