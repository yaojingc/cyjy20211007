...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggClsscheduleHVO[] commitVOs,
  ...
  protected Object callActionScript(AggClsscheduleHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggClsscheduleHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggClsscheduleHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(ClsscheduleHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggClsscheduleHVO> bills = service.queryBillOfVOByCond(
              AggClsscheduleHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggClsscheduleHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggClsscheduleHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            ClsscheduleHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggClsscheduleHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(ClsscheduleHVOConst.CONST_BILL_STATUS);
    ...
    if (hasFlow) {
      result = this.callActionScript(commitVOs,
              ClsscheduleHVOConst.CONST_BILLTYPE_COST);
    }
    // 没有，则收回即弃审
    else {
