...
import nccloud.cy.cy.clsschedulehvo.cy.bean.PageQueryVO;
 ...
 */
public class ClsscheduleHVOQueryListAction implements ICommonAction {
	...
	public Object doAction(IRequest paramIRequest) {
		...
		try {
			...
			AggClsscheduleHVO[] aggvos = CommonUtil.getFinancingcostMaintainService().query(scheme);
			// 查询服务(scheme);
			if ((aggvos != null) && (aggvos.length > 0)) {
				// 3、处理分页
				String[] pks = this.getPks(aggvos);
				AggClsscheduleHVO[] bills = this.getDefaulePageBill(queryParam, aggvos);
	...
	private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
		AggClsscheduleHVO[] bills = (AggClsscheduleHVO[]) pagevo.getCurrentPageBills();
	...
	private PageQueryVO createNullPage() {
		...
		AggClsscheduleHVO[] bills = Constructor.construct(AggClsscheduleHVO.class, 0);
	 ...
	 * @return
	 */
	private AggClsscheduleHVO[] getDefaulePageBill(QueryTreeFormatVO info, AggClsscheduleHVO... aggvos) {
		...
		List<AggClsscheduleHVO> billLst = new ArrayList<AggClsscheduleHVO>();
		for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
			AggClsscheduleHVO bill = new AggClsscheduleHVO();

			bill.setParent(aggvos[i].getParent());
			billLst.add(bill);
		}
		return billLst.toArray(new AggClsscheduleHVO[0]);
	}
	 ...
	 * @return
	 */
	private String[] getPks(AggClsscheduleHVO... aggvos) {
		List<String> pks = new ArrayList<String>();
		for (AggClsscheduleHVO bill : aggvos) {
