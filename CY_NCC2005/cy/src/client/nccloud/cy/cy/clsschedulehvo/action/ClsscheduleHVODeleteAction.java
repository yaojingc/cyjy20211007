 ...
 */
public class ClsscheduleHVODeleteAction implements ICommonAction {
...
BillOperatorParam queryParam = this.getQueryParam(paramIRequest);
    try {
      ...
      AggClsscheduleHVO[] bills = this.queryBills(queryParam);
      // ts赋值
  ...
  protected String getActionCode() {
    return ClsscheduleHVOConst.CONST_ACTION_DELETE;
  }
   ...
   */
  private Object callActionScript(AggClsscheduleHVO... aggVOs)
      throws BusinessException {
...
BillOperatorParam queryParam =
   ...
   */
  private AggClsscheduleHVO[] queryBills(BillOperatorParam queryParam)
      throws MetaDataException {
    // 1、根据参数查询结果
    IMDPersistenceQueryService service = CommonUtil.getMDPersistenceQueryService();
    String wheresql = SQLUtil.buildSqlForIn(
        "pk_clsschedule", queryParam.getPks());

    Collection<AggClsscheduleHVO> bills =
        service.queryBillOfVOByCond(AggClsscheduleHVO.class, wheresql, true,
            false);
    if ((bills == null) || (bills.size() == 0)) {
      return null;
    }
    return bills.toArray(new AggClsscheduleHVO[0]);
  }
}
