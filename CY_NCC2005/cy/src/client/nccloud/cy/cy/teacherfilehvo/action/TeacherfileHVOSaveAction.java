 ...
 */
public class TeacherfileHVOSaveAction implements ICommonAction {
...
AggTeacherfileHVO vo = billCardOperator.toBill(paramIRequest);
      this.doBefore(vo);
      // 2、调用单据的保存动作脚本（savebase），得到保存后结果
AggTeacherfileHVO rtnObj = this.callActionScript(vo);
  ...
  protected String getActionCode() {
    return TeacherfileHVOConst.CONST_ACTION_SAVEBASE;
  }
   ...
   */
  private AggTeacherfileHVO callActionScript(AggTeacherfileHVO... aggVOs)
      throws BusinessException {

    String actionCode = this.getActionCode();
    String billType = TeacherfileHVOConst.CONST_BILLTYPE_COST;
    ...
    String wheresql =
        "pk_teachfile" + "='"
            + ((AggTeacherfileHVO) result[0]).getPrimaryKey() + "'";
    ...
    return bills.toArray(new AggTeacherfileHVO[0])[0];
  }
   ...
   * @param vo
   */
  private void doBefore(AggTeacherfileHVO vo) {
    ...
    if ((parentPk != null) && !"".equals(parentPk)) {
      ...
      if (allchildren != null) {
        for (CircularlyAccessibleValueObject obj : allchildren) {
          obj.setAttributeValue("pk_teachfile", parentPk);
        }
      }
    }
  }
