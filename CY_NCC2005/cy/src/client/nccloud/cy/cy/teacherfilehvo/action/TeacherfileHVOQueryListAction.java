...
import nccloud.cy.cy.teacherfilehvo.cy.bean.PageQueryVO;
 ...
 */
public class TeacherfileHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggTeacherfileHVO[] bills =
        (AggTeacherfileHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggTeacherfileHVO[] bills =
        Constructor.construct(AggTeacherfileHVO.class, 0);
   ...
   * @return
   */
  private AggTeacherfileHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggTeacherfileHVO... aggvos) {
    ...
    List<AggTeacherfileHVO> billLst = new ArrayList<AggTeacherfileHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggTeacherfileHVO bill = new AggTeacherfileHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggTeacherfileHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggTeacherfileHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggTeacherfileHVO bill : aggvos) {
