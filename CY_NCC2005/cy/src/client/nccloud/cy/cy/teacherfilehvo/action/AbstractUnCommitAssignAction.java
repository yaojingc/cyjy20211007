...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggTeacherfileHVO[] commitVOs,
  ...
  protected Object callActionScript(AggTeacherfileHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggTeacherfileHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggTeacherfileHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(TeacherfileHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggTeacherfileHVO> bills = service.queryBillOfVOByCond(
              AggTeacherfileHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggTeacherfileHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggTeacherfileHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            TeacherfileHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggTeacherfileHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(TeacherfileHVOConst.CONST_BILL_STATUS);
