...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggDailyHVO[] commitVOs,
  ...
  protected Object callActionScript(AggDailyHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggDailyHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggDailyHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(DailyHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggDailyHVO> bills = service.queryBillOfVOByCond(
              AggDailyHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggDailyHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggDailyHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            DailyHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggDailyHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(DailyHVOConst.CONST_BILL_STATUS);
