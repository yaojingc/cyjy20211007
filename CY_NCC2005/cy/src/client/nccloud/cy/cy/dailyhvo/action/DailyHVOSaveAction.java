 ...
 */
public class DailyHVOSaveAction implements ICommonAction {
...
AggDailyHVO rtnObj = this.callActionScript(vo);
...
return CommonUtil.displayCardManager(card, rtnObj);
  ...
  protected String getActionCode() {
    return DailyHVOConst.CONST_ACTION_SAVEBASE;
  }
   ...
   */
  private AggDailyHVO callActionScript(AggDailyHVO... aggVOs)
      throws BusinessException {

    String actionCode = this.getActionCode();
    String billType = DailyHVOConst.CONST_BILLTYPE_COST;
    ...
    String wheresql =
        "pk_daily" + "='"
            + ((AggDailyHVO) result[0]).getPrimaryKey() + "'";
    Collection<AggDailyHVO> bills =
        CommonUtil.getMDPersistenceQueryService().queryBillOfVOByCond(
            AggDailyHVO.class, wheresql, true, false);
    return bills.toArray(new AggDailyHVO[0])[0];
  }
   ...
   * @param vo
   */
  private void doBefore(AggDailyHVO vo) {
    ...
    if ((parentPk != null) && !"".equals(parentPk)) {
      ...
      if (allchildren != null) {
        for (CircularlyAccessibleValueObject obj : allchildren) {
          obj.setAttributeValue("pk_daily", parentPk);
        }
      }
    }
  }
