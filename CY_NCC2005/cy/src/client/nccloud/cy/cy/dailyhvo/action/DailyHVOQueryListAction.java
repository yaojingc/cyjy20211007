...
import nccloud.cy.cy.dailyhvo.cy.bean.PageQueryVO;
 ...
 */
public class DailyHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggDailyHVO[] bills =
        (AggDailyHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggDailyHVO[] bills =
        Constructor.construct(AggDailyHVO.class, 0);
   ...
   * @return
   */
  private AggDailyHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggDailyHVO... aggvos) {
    ...
    List<AggDailyHVO> billLst = new ArrayList<AggDailyHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggDailyHVO bill = new AggDailyHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggDailyHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggDailyHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggDailyHVO bill : aggvos) {
