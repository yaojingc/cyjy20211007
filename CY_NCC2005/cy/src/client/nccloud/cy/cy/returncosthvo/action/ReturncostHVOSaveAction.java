 ...
 */
public class ReturncostHVOSaveAction implements ICommonAction {
...
AggReturncostHVO vo = billCardOperator.toBill(paramIRequest);
      this.doBefore(vo);
      // 2、调用单据的保存动作脚本（savebase），得到保存后结果
AggReturncostHVO rtnObj = this.callActionScript(vo);
  ...
  protected String getActionCode() {
    return ReturncostHVOConst.CONST_ACTION_SAVEBASE;
  }
   ...
   */
  private AggReturncostHVO callActionScript(AggReturncostHVO... aggVOs)
      throws BusinessException {

    String actionCode = this.getActionCode();
    String billType = ReturncostHVOConst.CONST_BILLTYPE_COST;
    ...
    String wheresql =
        "pk_returncost" + "='"
            + ((AggReturncostHVO) result[0]).getPrimaryKey() + "'";
    ...
    return bills.toArray(new AggReturncostHVO[0])[0];
  }
   ...
   * @param vo
   */
  private void doBefore(AggReturncostHVO vo) {
    ...
    if ((parentPk != null) && !"".equals(parentPk)) {
      ...
      if (allchildren != null) {
        for (CircularlyAccessibleValueObject obj : allchildren) {
          obj.setAttributeValue("pk_returncost", parentPk);
        }
      }
    }
  }
