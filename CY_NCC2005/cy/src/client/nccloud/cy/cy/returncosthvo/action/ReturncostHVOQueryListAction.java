...
import nccloud.cy.cy.returncosthvo.cy.bean.PageQueryVO;
 ...
 */
public class ReturncostHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggReturncostHVO[] bills =
        (AggReturncostHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggReturncostHVO[] bills =
        Constructor.construct(AggReturncostHVO.class, 0);
   ...
   * @return
   */
  private AggReturncostHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggReturncostHVO... aggvos) {
    ...
    List<AggReturncostHVO> billLst = new ArrayList<AggReturncostHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggReturncostHVO bill = new AggReturncostHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggReturncostHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggReturncostHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggReturncostHVO bill : aggvos) {
