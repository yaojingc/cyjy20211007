...
import nccloud.vo.cy.cy.ReturncostHVOConst;
...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggReturncostHVO[] commitVOs,
  ...
  protected Object callActionScript(AggReturncostHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggReturncostHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggReturncostHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(ReturncostHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggReturncostHVO> bills = service.queryBillOfVOByCond(
              AggReturncostHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggReturncostHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggReturncostHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            ReturncostHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggReturncostHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(ReturncostHVOConst.CONST_BILL_STATUS);
    ...
    if (hasFlow) {
      result = this.callActionScript(commitVOs,
              ReturncostHVOConst.CONST_BILLTYPE_COST);
    }
    // 没有，则收回即弃审
    else {
