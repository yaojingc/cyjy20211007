...
import nccloud.cy.cy.plantravelhvo.cy.bean.PageQueryVO;
 ...
 */
public class PlantravelHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggPlantravelHVO[] bills =
        (AggPlantravelHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggPlantravelHVO[] bills =
        Constructor.construct(AggPlantravelHVO.class, 0);
   ...
   * @return
   */
  private AggPlantravelHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggPlantravelHVO... aggvos) {
    ...
    List<AggPlantravelHVO> billLst = new ArrayList<AggPlantravelHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggPlantravelHVO bill = new AggPlantravelHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggPlantravelHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggPlantravelHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggPlantravelHVO bill : aggvos) {
