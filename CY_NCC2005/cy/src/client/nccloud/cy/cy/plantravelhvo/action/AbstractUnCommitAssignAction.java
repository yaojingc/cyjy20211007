...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggPlantravelHVO[] commitVOs,
  ...
  protected Object callActionScript(AggPlantravelHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggPlantravelHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggPlantravelHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(PlantravelHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggPlantravelHVO> bills = service.queryBillOfVOByCond(
      ...
      vos = bills.toArray(new AggPlantravelHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggPlantravelHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            PlantravelHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggPlantravelHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(PlantravelHVOConst.CONST_BILL_STATUS);
    ...
    if (hasFlow) {
      result = this.callActionScript(commitVOs,
              PlantravelHVOConst.CONST_BILLTYPE_COST);
    }
    // 没有，则收回即弃审
    else {
