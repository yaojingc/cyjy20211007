 ...
 */
public class PlantravelHVODeleteAction implements ICommonAction {
...
BillOperatorParam queryParam = this.getQueryParam(paramIRequest);
    try {
      ...
      AggPlantravelHVO[] bills = this.queryBills(queryParam);
      // ts赋值
  ...
  protected String getActionCode() {
    return PlantravelHVOConst.CONST_ACTION_DELETE;
  }
   ...
   */
  private Object callActionScript(AggPlantravelHVO... aggVOs)
      throws BusinessException {
...
BillOperatorParam queryParam =
   ...
   */
  private AggPlantravelHVO[] queryBills(BillOperatorParam queryParam)
      throws MetaDataException {
    // 1、根据参数查询结果
    IMDPersistenceQueryService service = CommonUtil.getMDPersistenceQueryService();
    String wheresql = SQLUtil.buildSqlForIn(
        "pk_plantravel", queryParam.getPks());

    Collection<AggPlantravelHVO> bills =
        service.queryBillOfVOByCond(AggPlantravelHVO.class, wheresql, true,
            false);
    if ((bills == null) || (bills.size() == 0)) {
      return null;
    }
    return bills.toArray(new AggPlantravelHVO[0]);
  }
}
