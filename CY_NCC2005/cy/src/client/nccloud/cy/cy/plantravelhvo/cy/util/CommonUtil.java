...
package nccloud.cy.cy.plantravelhvo.cy.util;
...
public class CommonUtil {
  ...
  private static IPlantravelhvoMaintain maintain = null;
  ...
  public static String getBillTypeCode() {
    return PlantravelHVOConst.CONST_BILLTYPE_COST;
  }
...
/**
   ...
   * @return
   */
  public static synchronized IPlantravelhvoMaintain getFinancingcostMaintainService() {
    if (CommonUtil.maintain == null) {
      CommonUtil.maintain = ServiceLocator.find(IPlantravelhvoMaintain.class);
    }
  ...
  public static void setBillsTs(Map<String, String> pkMapTs,
    ...
    for (AbstractBill bill : bills) {
      ...
      bill.getParent().setAttributeValue(PlantravelHVOConst.CONST_TS, ts);
    }
  }
	 ...
	 * @return
	 */
	public static ExtBillCard displayCardManager(ExtBillCard billCard, AggPlantravelHVO bill) {
		PlantravelHVO hvo = bill.getParentVO();
		// def1：学校
	 ...
	 * @return
	 */
	public static Grid displayGridManager(Grid grid, AggPlantravelHVO[] bills) {
		if (null == grid) {
			return grid;
		}
		Set<String> setdef1 = new HashSet<String>();
		for (AggPlantravelHVO aggvo : bills) {
			PlantravelHVO chvo = aggvo.getParentVO();
			// def1：学校
