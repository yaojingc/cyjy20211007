 ...
 */
public class CreditcontractHVOQueryByPksAction implements ICommonAction {
  ...
  protected Object transGrid(PageQueryInfo info,
    ...
    int i = 0;
    for (AggCreditcontractHVO bill : bills) {
      heads[i++] = bill.getParent();
    }

    GridOperator operator = new GridOperator(info.getPagecode());
    Grid grid = operator.toGrid(heads);
    grid = CommonUtil.displayGridManager(grid, bills.toArray(new AggCreditcontractHVO[0]));
    return grid;
  }
   ...
   */
  private Collection<AggCreditcontractHVO> queryBills(PageQueryInfo queryParam)
      throws MetaDataException {
    // 1、根据参数查询结果
    IMDPersistenceQueryService service = CommonUtil.getMDPersistenceQueryService();
    String wheresql =
        SQLUtil.buildSqlForIn("pk_contract",queryParam.getPks());
    Collection<AggCreditcontractHVO> bills =
        service.queryBillOfVOByCond(AggCreditcontractHVO.class, wheresql, false,
            false);
    return bills;
  }
}
