...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggCreditcontractHVO[] commitVOs,
  ...
  protected Object callActionScript(AggCreditcontractHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggCreditcontractHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggCreditcontractHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(CreditcontractHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggCreditcontractHVO> bills = service.queryBillOfVOByCond(
              AggCreditcontractHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggCreditcontractHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggCreditcontractHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            CreditcontractHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggCreditcontractHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(CreditcontractHVOConst.CONST_BILL_STATUS);
