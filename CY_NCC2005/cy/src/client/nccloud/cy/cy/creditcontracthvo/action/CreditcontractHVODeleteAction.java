 ...
 */
public class CreditcontractHVODeleteAction implements ICommonAction {
...
BillOperatorParam queryParam = this.getQueryParam(paramIRequest);
    try {
      ...
      AggCreditcontractHVO[] bills = this.queryBills(queryParam);
      // ts赋值
  ...
  protected String getActionCode() {
    return CreditcontractHVOConst.CONST_ACTION_DELETE;
  }
   ...
   */
  private Object callActionScript(AggCreditcontractHVO... aggVOs)
      throws BusinessException {
...
BillOperatorParam queryParam =
   ...
   */
  private AggCreditcontractHVO[] queryBills(BillOperatorParam queryParam)
      throws MetaDataException {
    // 1、根据参数查询结果
    IMDPersistenceQueryService service = CommonUtil.getMDPersistenceQueryService();
    String wheresql = SQLUtil.buildSqlForIn(
        "pk_contract", queryParam.getPks());

    Collection<AggCreditcontractHVO> bills =
        service.queryBillOfVOByCond(AggCreditcontractHVO.class, wheresql, true,
            false);
    if ((bills == null) || (bills.size() == 0)) {
      return null;
    }
    return bills.toArray(new AggCreditcontractHVO[0]);
  }
}
