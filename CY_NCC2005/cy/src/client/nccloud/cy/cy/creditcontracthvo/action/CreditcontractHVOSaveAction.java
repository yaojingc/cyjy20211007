 ...
 */
public class CreditcontractHVOSaveAction implements ICommonAction {
	...
	public Object doAction(IRequest paramIRequest) {
		try {
			...
			AggCreditcontractHVO vo = billCardOperator.toBill(paramIRequest);
			this.doBefore(vo);
			// 2、调用单据的保存动作脚本（savebase），得到保存后结果
			AggCreditcontractHVO rtnObj = this.callActionScript(vo);
	...
	protected String getActionCode() {
		return CreditcontractHVOConst.CONST_ACTION_SAVEBASE;
	}
	 ...
	 */
	private AggCreditcontractHVO callActionScript(AggCreditcontractHVO... aggVOs) throws BusinessException {

		String actionCode = this.getActionCode();
		String billType = CreditcontractHVOConst.CONST_BILLTYPE_COST;
		...
		String wheresql = "pk_contract" + "='" + ((AggCreditcontractHVO) result[0]).getPrimaryKey() + "'";
		Collection<AggCreditcontractHVO> bills = CommonUtil.getMDPersistenceQueryService()
				.queryBillOfVOByCond(AggCreditcontractHVO.class, wheresql, true, false);
		return bills.toArray(new AggCreditcontractHVO[0])[0];
	}
	 ...
	 * @param vo
	 */
	private void doBefore(AggCreditcontractHVO vo) {
		...
		if ((parentPk != null) && !"".equals(parentPk)) {
			...
			if (allchildren != null) {
				for (CircularlyAccessibleValueObject obj : allchildren) {
					obj.setAttributeValue("pk_contract", parentPk);
				}
			}
		}
	}
