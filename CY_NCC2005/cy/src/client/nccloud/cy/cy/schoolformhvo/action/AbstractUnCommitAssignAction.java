...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggSchoolformHVO[] commitVOs,
  ...
  protected Object callActionScript(AggSchoolformHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggSchoolformHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggSchoolformHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(SchoolformHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggSchoolformHVO> bills = service.queryBillOfVOByCond(
      ...
      vos = bills.toArray(new AggSchoolformHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggSchoolformHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            SchoolformHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggSchoolformHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(SchoolformHVOConst.CONST_BILL_STATUS);
