...
import nccloud.cy.cy.schoolformhvo.cy.bean.PageQueryVO;
 ...
 */
public class SchoolformHVOQueryListAction implements ICommonAction {
	...
	public Object doAction(IRequest paramIRequest) {
		...
		try {
			...
			AggSchoolformHVO[] aggvos = CommonUtil.getFinancingcostMaintainService().query(scheme);
			// 查询服务(scheme);
			if ((aggvos != null) && (aggvos.length > 0)) {
				// 3、处理分页
				String[] pks = this.getPks(aggvos);
				AggSchoolformHVO[] bills = this.getDefaulePageBill(queryParam, aggvos);
	...
	private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
		AggSchoolformHVO[] bills = (AggSchoolformHVO[]) pagevo.getCurrentPageBills();
	...
	private PageQueryVO createNullPage() {
		...
		AggSchoolformHVO[] bills = Constructor.construct(AggSchoolformHVO.class, 0);
	 ...
	 * @return
	 */
	private AggSchoolformHVO[] getDefaulePageBill(QueryTreeFormatVO info, AggSchoolformHVO... aggvos) {
		...
		List<AggSchoolformHVO> billLst = new ArrayList<AggSchoolformHVO>();
		for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
			AggSchoolformHVO bill = new AggSchoolformHVO();

			bill.setParent(aggvos[i].getParent());
			billLst.add(bill);
		}
		return billLst.toArray(new AggSchoolformHVO[0]);
	}
	 ...
	 * @return
	 */
	private String[] getPks(AggSchoolformHVO... aggvos) {
		List<String> pks = new ArrayList<String>();
		for (AggSchoolformHVO bill : aggvos) {
