 ...
 */
public class SchoolformHVOQueryByPksAction implements ICommonAction {
  ...
  protected Object transGrid(PageQueryInfo info,
    ...
    int i = 0;
    for (AggSchoolformHVO bill : bills) {
...
//    Grid grid = operator.toGrid(heads);
    GridOperator operator = new GridOperator(info.getPagecode());
    Grid grid = operator.toGrid(heads);
	grid = CommonUtil.displayGridManager(grid, bills.toArray(new AggSchoolformHVO[0]));
    return grid;
  }
   ...
   */
  private Collection<AggSchoolformHVO> queryBills(PageQueryInfo queryParam)
      throws MetaDataException {
    // 1、根据参数查询结果
    IMDPersistenceQueryService service = CommonUtil.getMDPersistenceQueryService();
    String wheresql =
        SQLUtil.buildSqlForIn("pk_schoolf",queryParam.getPks());
    Collection<AggSchoolformHVO> bills =
        service.queryBillOfVOByCond(AggSchoolformHVO.class, wheresql, false,
            false);
    return bills;
  }
}
