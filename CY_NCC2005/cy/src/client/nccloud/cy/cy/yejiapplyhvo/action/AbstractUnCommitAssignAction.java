...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggYejiapplyHVO[] commitVOs,
  ...
  protected Object callActionScript(AggYejiapplyHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggYejiapplyHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggYejiapplyHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(YejiapplyHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggYejiapplyHVO> bills = service.queryBillOfVOByCond(
              AggYejiapplyHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggYejiapplyHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggYejiapplyHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            YejiapplyHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggYejiapplyHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(YejiapplyHVOConst.CONST_BILL_STATUS);
    ...
    if (hasFlow) {
      result = this.callActionScript(commitVOs,
              YejiapplyHVOConst.CONST_BILLTYPE_COST);
    }
    // 没有，则收回即弃审
    else {
