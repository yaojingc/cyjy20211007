 ...
 */
public class YejiapplyHVODeleteAction implements ICommonAction {
...
BillOperatorParam queryParam = this.getQueryParam(paramIRequest);
    try {
      ...
      AggYejiapplyHVO[] bills = this.queryBills(queryParam);
      // ts赋值
  ...
  protected String getActionCode() {
    return YejiapplyHVOConst.CONST_ACTION_DELETE;
  }
   ...
   */
  private Object callActionScript(AggYejiapplyHVO... aggVOs)
      throws BusinessException {
...
BillOperatorParam queryParam =
   ...
   */
  private AggYejiapplyHVO[] queryBills(BillOperatorParam queryParam)
      throws MetaDataException {
    // 1、根据参数查询结果
    IMDPersistenceQueryService service = CommonUtil.getMDPersistenceQueryService();
    String wheresql = SQLUtil.buildSqlForIn(
        "pk_yejiapply", queryParam.getPks());

    Collection<AggYejiapplyHVO> bills =
        service.queryBillOfVOByCond(AggYejiapplyHVO.class, wheresql, true,
            false);
    if ((bills == null) || (bills.size() == 0)) {
      return null;
    }
    return bills.toArray(new AggYejiapplyHVO[0]);
  }
}
