...
import nccloud.cy.cy.yejiapplyhvo.cy.bean.PageQueryVO;
 ...
 */
public class YejiapplyHVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggYejiapplyHVO[] bills =
        (AggYejiapplyHVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggYejiapplyHVO[] bills =
        Constructor.construct(AggYejiapplyHVO.class, 0);
   ...
   * @return
   */
  private AggYejiapplyHVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggYejiapplyHVO... aggvos) {
    ...
    List<AggYejiapplyHVO> billLst = new ArrayList<AggYejiapplyHVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggYejiapplyHVO bill = new AggYejiapplyHVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggYejiapplyHVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggYejiapplyHVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggYejiapplyHVO bill : aggvos) {
