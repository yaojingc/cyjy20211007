 ...
 */
public class YejiapplyHVOSaveAction implements ICommonAction {
	...
	public Object doAction(IRequest paramIRequest) {
		try {
			...
			AggYejiapplyHVO vo = billCardOperator.toBill(paramIRequest);
			this.doBefore(vo);
			// 2、调用单据的保存动作脚本（savebase），得到保存后结果
			AggYejiapplyHVO rtnObj = this.callActionScript(vo);
...
// 4、返回结果到前端
	...
	protected String getActionCode() {
		return YejiapplyHVOConst.CONST_ACTION_SAVEBASE;
	}
	 ...
	 */
	private AggYejiapplyHVO callActionScript(AggYejiapplyHVO... aggVOs) throws BusinessException {

		String actionCode = this.getActionCode();
		String billType = YejiapplyHVOConst.CONST_BILLTYPE_COST;
		...
		String wheresql = "pk_yejiapply" + "='" + ((AggYejiapplyHVO) result[0]).getPrimaryKey() + "'";
		Collection<AggYejiapplyHVO> bills = CommonUtil.getMDPersistenceQueryService()
				.queryBillOfVOByCond(AggYejiapplyHVO.class, wheresql, true, false);
		return bills.toArray(new AggYejiapplyHVO[0])[0];
	}
	 ...
	 * @param vo
	 */
	private void doBefore(AggYejiapplyHVO vo) {
		...
		if ((parentPk != null) && !"".equals(parentPk)) {
			...
			if (allchildren != null) {
				for (CircularlyAccessibleValueObject obj : allchildren) {
					obj.setAttributeValue("pk_yejiapply", parentPk);
				}
			}
		}
	}
