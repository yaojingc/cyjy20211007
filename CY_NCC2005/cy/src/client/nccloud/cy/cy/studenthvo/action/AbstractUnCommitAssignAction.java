...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggStudentHVO[] commitVOs,
  ...
  protected Object callActionScript(AggStudentHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggStudentHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggStudentHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(StudentHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggStudentHVO> bills = service.queryBillOfVOByCond(
              AggStudentHVO.class, wheresql, true, false);
      if ((bills == null) || (bills.size() != pks.length)) {
        ExceptionUtils.wrapBusinessException("单据已被修改，请刷新页面后重试");
      }
      vos = bills.toArray(new AggStudentHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggStudentHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            StudentHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggStudentHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(StudentHVOConst.CONST_BILL_STATUS);
