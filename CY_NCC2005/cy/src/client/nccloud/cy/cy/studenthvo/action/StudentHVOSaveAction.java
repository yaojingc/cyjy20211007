 ...
 */
public class StudentHVOSaveAction implements ICommonAction {
...
//      BillCardOperator billCardOperator = new BillCardOperator();
			...
			AggStudentHVO rtnObj = this.callActionScript(vo);
			// 3、处理返回结果（包含功能：根据模板转换前端BillCard，参照翻译，显示公式处理）
			ExtBillCard billcard = extbillCardOperator.toCard(rtnObj);
			// 4、返回结果到前端
			// 自定义参照翻译
	...
	protected String getActionCode() {
		return StudentHVOConst.CONST_ACTION_SAVEBASE;
	}
	 ...
	 */
	private AggStudentHVO callActionScript(AggStudentHVO... aggVOs) throws BusinessException {
		...
		String wheresql = "pk_student" + "='" + ((AggStudentHVO) result[0]).getPrimaryKey() + "'";
		Collection<AggStudentHVO> bills = CommonUtil.getMDPersistenceQueryService()
				.queryBillOfVOByCond(AggStudentHVO.class, wheresql, true, false);
		return bills.toArray(new AggStudentHVO[0])[0];
	}
	 ...
	 * @param vo
	 */
	private void doBefore(AggStudentHVO vo) {
		...
		if ((parentPk != null) && !"".equals(parentPk)) {
			...
			if (allchildren != null) {
				for (CircularlyAccessibleValueObject obj : allchildren) {
					obj.setAttributeValue("pk_student", parentPk);
				}
			}
		}
	}
