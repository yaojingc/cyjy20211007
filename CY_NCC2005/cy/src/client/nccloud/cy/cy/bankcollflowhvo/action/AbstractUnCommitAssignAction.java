...
public abstract class AbstractUnCommitAssignAction<T extends AbstractBill>
  ...
  public Object callAutoUnApproveActionScript(AggBankcollflowHVO[] commitVOs,
  ...
  protected Object callActionScript(AggBankcollflowHVO[] commitVOs,
                                    String billType) throws BusinessException {
  ...
  protected AggBankcollflowHVO[] queryBillsByPks(BillOperatorParam operaParam)
          throws BusinessException {
    AggBankcollflowHVO[] vos = null;
    ...
    if (pks.length != 0) {
      ...
      String wheresql = SQLUtil
              .buildSqlForIn(BankcollflowHVOConst.CONST_PARENT_PRIMARYKEY, pks);
      Collection<AggBankcollflowHVO> bills = service.queryBillOfVOByCond(
      ...
      vos = bills.toArray(new AggBankcollflowHVO[0]);
    }
    return vos;
  }

  protected Object unCommit(AggBankcollflowHVO[] commitVOs)
          throws BusinessException {
    Object result = null;
    // 判断是否定义审批流
    boolean hasFlow = NCCFlowUtils.hasApproveflowDef(
            BankcollflowHVOConst.CONST_BILLTYPE_COST, commitVOs[0]);
    for (AggBankcollflowHVO vo : commitVOs) {
      Integer vbillstatus = (Integer) vo.getParent()
              .getAttributeValue(BankcollflowHVOConst.CONST_BILL_STATUS);
    ...
    if (hasFlow) {
      result = this.callActionScript(commitVOs,
              BankcollflowHVOConst.CONST_BILLTYPE_COST);
    }
    // 没有，则收回即弃审
    else {
