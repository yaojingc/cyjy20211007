...
import nccloud.cy.cy.bankcollflowhvo.cy.bean.PageQueryVO;
 ...
 */
public class BankcollflowHVOQueryListAction implements ICommonAction {
	...
	public Object doAction(IRequest paramIRequest) {
		...
		try {
			...
			AggBankcollflowHVO[] aggvos = CommonUtil.getFinancingcostMaintainService().query(scheme);
			// 查询服务(scheme);
			if ((aggvos != null) && (aggvos.length > 0)) {
				// 3、处理分页
				String[] pks = this.getPks(aggvos);
				AggBankcollflowHVO[] bills = this.getDefaulePageBill(queryParam, aggvos);
	...
	private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
		AggBankcollflowHVO[] bills = (AggBankcollflowHVO[]) pagevo.getCurrentPageBills();
	...
	private PageQueryVO createNullPage() {
		...
		AggBankcollflowHVO[] bills = Constructor.construct(AggBankcollflowHVO.class, 0);
	 ...
	 * @return
	 */
	private AggBankcollflowHVO[] getDefaulePageBill(QueryTreeFormatVO info, AggBankcollflowHVO... aggvos) {
		...
		List<AggBankcollflowHVO> billLst = new ArrayList<AggBankcollflowHVO>();
		for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
			AggBankcollflowHVO bill = new AggBankcollflowHVO();

			bill.setParent(aggvos[i].getParent());
			billLst.add(bill);
		}
		return billLst.toArray(new AggBankcollflowHVO[0]);
	}
	 ...
	 * @return
	 */
	private String[] getPks(AggBankcollflowHVO... aggvos) {
		List<String> pks = new ArrayList<String>();
		for (AggBankcollflowHVO bill : aggvos) {
