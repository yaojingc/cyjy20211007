 ...
 */
public class StumarkdataHVOQueryByPksAction implements ICommonAction {
	 ...
	 * @return
	 */
	protected Object transGrid(PageQueryInfo info, Collection<AggStumarkdataHVO> bills) {
		...
		int i = 0;
		for (AggStumarkdataHVO bill : bills) {
			heads[i++] = bill.getParent();
		}

		GridOperator operator = new GridOperator(info.getPagecode());
		Grid grid = operator.toGrid(heads);
		grid = CommonUtil.displayGridManager(grid, bills.toArray(new AggStumarkdataHVO[0]));
		return grid;
	}
	...
	private Collection<AggStumarkdataHVO> queryBills(PageQueryInfo queryParam) throws MetaDataException {
		...
		String wheresql = SQLUtil.buildSqlForIn("pk_stumark", queryParam.getPks());
		Collection<AggStumarkdataHVO> bills = service.queryBillOfVOByCond(AggStumarkdataHVO.class, wheresql, false,
				false);
		return bills;
	}
}
