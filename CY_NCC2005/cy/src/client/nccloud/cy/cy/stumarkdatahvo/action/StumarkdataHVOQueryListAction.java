...
import nccloud.cy.cy.stumarkdatahvo.cy.bean.PageQueryVO;
 ...
 */
public class StumarkdataHVOQueryListAction implements ICommonAction {
	...
	public Object doAction(IRequest paramIRequest) {
		...
		try {
			...
			AggStumarkdataHVO[] aggvos = CommonUtil.getFinancingcostMaintainService().query(scheme);
			// 查询服务(scheme);
			if ((aggvos != null) && (aggvos.length > 0)) {
				// 3、处理分页
				String[] pks = this.getPks(aggvos);
				AggStumarkdataHVO[] bills = this.getDefaulePageBill(queryParam, aggvos);
	...
	private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
		AggStumarkdataHVO[] bills = (AggStumarkdataHVO[]) pagevo.getCurrentPageBills();
	...
	private PageQueryVO createNullPage() {
		...
		AggStumarkdataHVO[] bills = Constructor.construct(AggStumarkdataHVO.class, 0);
	 ...
	 * @return
	 */
	private AggStumarkdataHVO[] getDefaulePageBill(QueryTreeFormatVO info, AggStumarkdataHVO... aggvos) {
		...
		List<AggStumarkdataHVO> billLst = new ArrayList<AggStumarkdataHVO>();
		for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
			AggStumarkdataHVO bill = new AggStumarkdataHVO();

			bill.setParent(aggvos[i].getParent());
			billLst.add(bill);
		}
		return billLst.toArray(new AggStumarkdataHVO[0]);
	}
	 ...
	 * @return
	 */
	private String[] getPks(AggStumarkdataHVO... aggvos) {
		List<String> pks = new ArrayList<String>();
		for (AggStumarkdataHVO bill : aggvos) {
