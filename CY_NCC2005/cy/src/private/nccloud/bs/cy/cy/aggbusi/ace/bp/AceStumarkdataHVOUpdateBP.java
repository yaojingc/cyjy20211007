 ...
 *
 */
public class AceStumarkdataHVOUpdateBP {

	public AggStumarkdataHVO[] update(AggStumarkdataHVO[] bills, AggStumarkdataHVO[] originBills) {
		// 调用修改模板
		UpdateBPTemplate<AggStumarkdataHVO> bp = new UpdateBPTemplate<AggStumarkdataHVO>(
				StumarkdataHVOPluginPoint.UPDATE);
		// 执行前规则
	...
	private void addBeforeRule(CompareAroundProcesser<AggStumarkdataHVO> processer) {
		...
		((nc.bs.pubapp.pub.rule.UpdateBillCodeRule) ruleCom).setCbilltype("XSCJ");
