...
public class RSAUtil {
     ...
     * @return
     */
    public static String[] genRSAKeyPair(int keyLength) {
        KeyPairGenerator keyGen = null;
        try {
            keyGen = KeyPairGenerator.getInstance("RSA");
        } catch (NoSuchAlgorithmException e) {
            throw new RuntimeException("程序内部错误" + e.getMessage());
        }
        keyGen.initialize(keyLength);
        KeyPair keyPair = keyGen.generateKeyPair();
        try {
            String privateKeyStr = new String(java.util.Base64.getEncoder().encode(keyPair.getPrivate().getEncoded()), "UTF8");
            String publicKeyStr = new String(java.util.Base64.getEncoder().encode(keyPair.getPublic().getEncoded()), "UTF8");
